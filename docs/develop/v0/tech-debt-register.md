...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.256 Phase 2a scaffolding)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK Phase 1 fixed (unify arg order) + Phase 2a scaffolding done (expected_ty param added to lower_expr_to_operand + lower_expr_to_place, all 51 call sites pass None). Phase 2b-2f planned in `plan-18.255.md` §4.2.3 — 5 more stages (18.257-18.261, ~430 LOC remaining). New TD-UNIFY-ARG-ORDER (P3) registered for same-class unify arg order issues in typeck/check.rs (5 sites, batched fix planned in Stage 18.262+).
...
| TD-TUPLE-CTOR-TYPECK | type checker 对 generic tuple struct ctor 宽松 | No generic instantiation validation | 🟡 Phase 1+2a RESOLVED Stage 18.255-18.256. Phase 1 (18.255): unify arg order swap, error message direction corrected. Phase 2a (18.256): expected_ty: Option<&Ty> param added to lower_expr_to_operand + lower_expr_to_place (scaffolding, all 51 call sites pass None). Phase 2b-f DEFERRED to Stage 18.257-18.261 — Stage 18.233 audit found root cause: tuple struct ctor calls create a temp local, losing the expected type context. Fix requires expected-type propagation through MIR lower. Plan: 5 more stages (18.257-18.261) documented in `plan-18.255.md` §4.2.3. Regression tests: `tests/v0/stage18/plan/stage18_255_td_tuple_ctor_typeck_regression_tests.rs` (10 tests) + `stage18_256_phase2a_scaffolding_tests.rs` (8 tests). |
