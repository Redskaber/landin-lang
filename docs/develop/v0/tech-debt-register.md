...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.258 — Phase 2c closes soundness hole)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK Phase 1 (unify arg order) + Phase 2a (scaffolding) + Phase 2b (thread from let) + Phase 2c (use in lower_call_expr) all RESOLVED. Soundness hole CLOSED. Phases 2d-2f (return expr + method call + tests cleanup) deferred as optional improvements — current fix already closes the soundness hole. New TD-UNIFY-ARG-ORDER (P3) registered for same-class unify arg order issues in typeck/check.rs (5 sites, batched fix planned in Stage 18.262+).
