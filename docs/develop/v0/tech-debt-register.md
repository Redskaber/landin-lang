...
> **Author**: redskaber
> **Date**: 2026-08-29 (last updated Stage 18.376 — TD-ARCH-NESTED-GENERIC-FIELD-ACCESS fully resolved)
> **Version**: v0.510.0
> **Status**: Stage 18.376 fully resolved TD-ARCH-NESTED-GENERIC-FIELD-ACCESS — nested generic field access `Outer<Inner<T>>.inner.val` now compiles and runs correctly. Three-layer root-cause fix: (1) `resolve_adt_field_tys` now uses `lower_hir_ty_to_mir_ty_with_generics` so nested `T` resolves to `Param(0)` not `Error`; (2) `lower_hir_ty_to_mir_ty_with_generics_and_regions` delegates to full implementation instead of duplicating Path arm that missed `Res::GenericParam`; (3) struct literal inference uses recursive `collect_param_bindings` to extract Param from nested generic field types (Adt/Ref/RawPtr/Array/Tuple); (4) writeback applies `substitute` to `AggregateKind::Adt` field_tys; (5) `collect_from_aggregate_kind` adds `substs_are_concrete` check to skip generic definitions. **ALL P0/P1/P2 TDs RESOLVED.** Only BLOCKED TDs require v0.5+ architecture work: TD-INTRINSIC-OVERUSE Phase 2-B/C (needs fat pointer syntax), TD-STUB-PRELUDE-LOOP-BODY (same). 4409 tests (682 lib + 3727 integration), 0 failures (single-thread, ulimit -s unlimited). fmt clean, 0 clippy warnings. v0.4 release-ready.
