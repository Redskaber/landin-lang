...
| TD-GENERIC-FN-RETURN-EXPECTED-TY | When generic tuple struct ctor with wrong type is used in fn body return position (e.g., `fn make() -> Holder<i32> { Holder(true) }`), soundness hole remains. Root cause: `expected_ty` from fn sig return type is not threaded into fn body's `lower_expr_to_operand` calls. | `fn make() -> Holder<i32> { Holder(true) }` does NOT error — soundness hole in fn body return path. | 🟡 PARTIAL (Stage 18.269) — Phase 2d implemented: `expected_ty = return_mir_ty` now threaded into body tail expression lowering (src/mir/lower/body_lower.rs line ~411). However, the deeper issue is that `return_mir_ty` for `Holder<i32>` is lowered as `Adt(holder_def, [])` (empty substs) because the HIR return type path resolution doesn't extract substs when the path is used in a fn sig context. Fix requires investigating `lower_hir_ty_to_mir_ty_with_lifetimes` path resolution for fn sig return types. Per §17.6 "直到审查不出问题为止": continued audit needed. Documented as MVP — workaround is explicit turbofish (`Holder::<i32>(true)`). |
