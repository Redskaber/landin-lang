...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.264 — TD-STRUCT-LITERAL-FIELD-EXPECTED-TY + TD-BOX-NEW-EXPECTED-TY resolved)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK fully RESOLVED (Phases 1+2a+2b+2c, Stages 18.255-18.258). TD-UNIFY-ARG-ORDER (P3) RESOLVED Stage 18.259. TD-TUPLE-CTOR-CALL-ARG (Phase 2e) RESOLVED Stage 18.262. TD-STRUCT-LITERAL-FIELD-EXPECTED-TY + TD-BOX-NEW-EXPECTED-TY RESOLVED Stage 18.264 (holistic audit per §17.6 — when one bug is found, similar bugs hide together). Soundness hole FULLY CLOSED across all expression contexts (let binding, return expr, if/else, match, array, fn call args, struct literal fields, Box::new intrinsic). 3865 tests, 0 failures. Remaining: TD-INTRINSIC-OVERUSE Phase 2 (blocked on v0.4+ language features), TD-DROP-MOVED-LOCALS full (deferred to v0.3+ flow-sensitive tracking).
