...
> **Author**: redskaber
> **Date**: 2026-08-29 (last updated Stage 18.377 — TD-ALLOW-SUPPRESSION audit)
> **Version**: v0.510.0
> **Status**: Stage 18.377 closed TD-ALLOW-SUPPRESSION — audited 26 production `#[allow(...)]` suppressions across codebase. 6 stale allows removed (5 unused_imports in driver/mod.rs + 1 dead_code in typeck/unify.rs covering a truly unused `int_to_uint` function which was deleted). Remaining 20 allows verified as legitimate: (1) `#[allow(dead_code)]` on `region_inference` mod — 13 dead code warnings for SCC/universe/type-test infrastructure BLOCKED on TD-STUB-REGION-ERASED (v0.2+ NLL); (2) `#[allow(deprecated)]` on `ty_is_copy` — test backward compat; (3) `#[allow(clippy::too_many_arguments)]` (4 sites) — codegen context requires many params, v0.5+ Phase 1 will introduce CodegenCtxt struct; (4) `#[allow(clippy::only_used_in_recursion)]` (3 sites) — forward-compat API consistency; (5) `#[allow(clippy::collapsible_match)]` (2 sites) — style preference; (6) `#[allow(clippy::should_implement_trait)]` on `TargetTriple::from_str` — should be `FromStr` impl, tracked as minor TD; (7) other singletons (module_inception, enum_variant_names, arc_with_non_send_sync, while_let_loop, unreachable_patterns) — all legitimate. **ALL P0/P1/P2 TDs RESOLVED.** Only BLOCKED TDs require v0.5+ architecture work: TD-INTRINSIC-OVERUSE Phase 2-B/C, TD-STUB-PRELUDE-LOOP-BODY. 4409 tests (682 lib + 3727 integration), 0 failures (single-thread, ulimit -s unlimited). fmt clean, 0 clippy warnings. v0.4 release-ready.
