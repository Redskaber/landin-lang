...
| TD-ARCH-NESTED-GENERIC-FIELD-ACCESS | 嵌套泛型结构体字段访问 (`Outer<Inner<T>>` where `Outer<T> { inner: Inner<T> }` and `Inner<T> { ptr: *mut T }`) 报 false "expected *mut i64, found *mut <type param>". | 根因: Phase 3.7 writeback 的 fixpoint loop 只解析了 `o.inner` 的类型 (第一层), 但 `o.inner.ptr` 的结果 local 在 lower 时被设置为 `RawPtr(Mutable, Param(0))` — 需要 `o.inner` 的类型先解析为 `Adt(Inner, [i64])` 才能解析 `o.inner.ptr` 的 `RawPtr(Mutable, Param(0))` → `RawPtr(Mutable, i64)`. Phase 3.7 的 fixpoint 应该能处理这个 (每次迭代解析一层), 但 `o.inner.ptr` 的结果 local 可能被 Phase 3.5 的 FieldTyTable 覆盖回未替换状态, 且 Phase 3.7 的单次调用不足以修复两层 regression. | `src/typeck/checker.rs:check_mir_body_with_tables (Phase 3.7)` + `src/typeck/writeback.rs:writeback_field_types_with_table (Phase 3.5)` | architecture limitation | 🟡 v0.5+ 架构级重构 — Phase 3.5 `writeback_field_types_with_table` 用 FieldTyTable 中的未替换 HIR 类型覆盖 writeback 结果. 当前 Phase 3.7 的单次 re-writeback 能修复单层 (Stage 18.355 Holder<T> case), 但嵌套 (`Outer<Inner<T>>`) 需要: (1) Phase 3.5 不覆盖已解析的 field_ty; 或 (2) Phase 3.7 在 fixpoint loop 中运行直到收敛. Per §1.0 原則 6 (通解 > 特解): Phase 3.5 应该在覆盖 field_ty 时应用 substitute(). Per §12 (最优 > 最小): 根因在 Phase 3.5 的 table overwrite, Phase 3.7 是 workaround. 纳入 v0.5+ 架构级重构计划. |
