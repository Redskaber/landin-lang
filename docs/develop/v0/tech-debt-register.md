...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.262 — Phase 2e closes call-arg soundness hole)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK fully RESOLVED (Phases 1+2a+2b+2c, Stages 18.255-18.258). TD-UNIFY-ARG-ORDER (P3) RESOLVED Stage 18.259. TD-TUPLE-CTOR-CALL-ARG (Phase 2e) RESOLVED Stage 18.262 — fn_sigs propagated into MIR lower as read-only data contract, lower_call_expr threads expected_ty from sig.inputs[i] into call args. Soundness hole FULLY CLOSED (all 5 cases). Remaining: TD-INTRINSIC-OVERUSE Phase 2 (blocked on v0.4+ language features), TD-DROP-MOVED-LOCALS full (deferred to v0.3+ flow-sensitive tracking).
