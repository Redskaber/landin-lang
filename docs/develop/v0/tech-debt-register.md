...
| TD-BOX-AUTO-DROP | Box 缺 Box::new sugar + auto-drop | Box 使用不便, 内存泄漏风险 | ✅ Resolved Stage 18.193 + 18.212 — Box<T> auto-deallocates via drop glue (emit_drop_glue_functions handles Box specially: loads field 0, null-check, calls __landin_dealloc). Users no longer need manual dealloc. |
| TD-TUPLE-CTOR-TYPECK | type checker 对 generic tuple struct ctor 宽松 (Box(*mut u8) 接受为 Box<i32>) | 类型安全漏洞 | ✅ Resolved Stage 18.212 — same fix as above (duplicate entry) |
...
| TD-BOX-AUTO-DROP | Box 无自动释放 | drop elaboration 不跟踪 moved-from locals | ✅ Resolved Stage 18.193 + 18.212 — Box<T> auto-drop works via drop glue with null-check workaround. Full move tracking (TD-DROP-MOVED-LOCALS) deferred to v0.3. |
...
| TD-INT-UINT-VAR | typeck Int/Uint 变量统一 | unify table 丢失 Int↔Uint 区别 | 🟡 Partial Stage 18.213 — Vec<T>::push unsuffixed literal fix (extract element type from Vec substs). Full IntOrUintVar separation still v0.2 P2+. |
| TD-TUPLE-CTOR-TYPECK | type checker 对 generic tuple struct ctor 宽松 | No generic instantiation validation | ✅ Resolved Stage 18.212 — same fix as above (duplicate entry) |
