...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.255 TD-TUPLE-CTOR-TYPECK Phase 1 + audit)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK Phase 1 fixed (unify arg order). Phase 2 (expected-type propagation) planned in `plan-18.255.md` §4.2 — 6 stages (18.256-18.261, ~530 LOC). New TD-UNIFY-ARG-ORDER (P3) registered for same-class unify arg order issues in typeck/check.rs (5 sites, batched fix planned in Stage 18.262+).
...
| TD-TUPLE-CTOR-TYPECK | type checker 对 generic tuple struct ctor 宽松 (Box(*mut u8) 接受为 Box<i32>) | 类型安全漏洞 | 🟡 Deferred v0.3+ — Stage 18.233 audit: root cause is MIR lower creates temp local losing expected type context. Fix requires expected-type propagation (~500 LOC architecture change). |
...
| TD-DROP-MOVED-LOCALS | drop elaboration 缺少 move tracking | No move-state tracking | 🟡 Partial — Stage 18.243: `collect_moved_locals` extended to scan Store/terminator/Load/GEP moves. Flow-insensitive (may over-approximate). Full flow-sensitive tracking deferred to v0.3+. |
...
| TD-TUPLE-CTOR-TYPECK | type checker 对 generic tuple struct ctor 宽松 | No generic instantiation validation | 🟡 Phase 1 RESOLVED Stage 18.255 (unify arg order swap, error message direction corrected). Phase 2 DEFERRED to Stage 18.256+ — Stage 18.233 audit found root cause: tuple struct ctor calls create a temp local, losing the expected type context. Fix requires expected-type propagation through MIR lower (threading `expected_ty: Option<&Ty>` through all `lower_expr_*` functions). Plan: 6 stages (18.256-18.261) documented in `plan-18.255.md` §4.2. Phase 1 fixes error message direction for turbofish + Array cases. Phase 2 fixes soundness hole when no turbofish + let annotation. Per §17.6 documented as deferred MVP. Regression tests: `tests/v0/stage18/plan/stage18_255_td_tuple_ctor_typeck_regression_tests.rs` (10 tests: 3 positive + 6 negative + 1 deferred-MVP marker). |
...
| TD-INTRINSIC-OVERUSE | Stdlib methods (String::len, Vec::push, Box::new, etc.) implemented as hardcoded MIR lower intrinsics (8 method_name_str checks + 7 specialized functions + 11-entry typeck whitelist) instead of regular `impl` blocks in prelude source | 特解 pattern: 4+ files must be synced for each new type/method; scattered logic; violates §1.0 原則 6 (通解 > 特解); caused TD-TUPLE-CTOR-TYPECK + TD-METHOD-RESOLVE-STRICT whitelist; same pattern class as TD-C-WRAPPER-OVERUSE (Phase 1 → Phase 2) | 🟡 Phase 1 done Stage 18.238 (Vec::len/new removed). Phase 2 BLOCKED: all remaining intrinsics need language features not yet implemented — (1) primitive type impl (impl str) for str::len/is_empty/as_bytes; (2) fat pointer construction syntax for String::as_str; (3) extern C in prelude impl for from_str/push_str/push/get/Box::new/format!. Deferred to v0.3 when these language features are available. Stage 18.239 audit documents all blockers. |
| TD-UNIFY-ARG-ORDER | 5 unify call sites in `typeck/check.rs` (Call arg, Call return, Switch discr) have swapped expected/found arg order — same class of bug as TD-TUPLE-CTOR-TYPECK Phase 1 fix | Error messages display reversed direction: "expected <actual>, found <declared>" instead of "expected <declared>, found <actual>" | 🟡 NEW — Stage 18.255 audit identified same-class errors in `typeck/check.rs:314,324,340,349,392,400,445`. Not fixed in 18.255 because: (1) risk of breaking existing tests that depend on specific error messages; (2) needs comprehensive test impact analysis; (3) per §17.6 holistic plan, batched in Stage 18.262+ (after TD-TUPLE-CTOR-TYPECK Phase 2 lands). Sites already fixed in 18.255: `typeck/infer.rs:543` (Array elem) + `typeck/infer.rs:568` (Adt field). Sites correct: `typeck/check.rs:229,236,238` (let binding — place is expected, rvalue is found). |
