...
> **Author**: redskaber
> **Date**: 2026-08-30 (last updated Stage 18.422 — §20 iterative audit: &str indexing rejection + as_bytes intrinsic fix)
> **Version**: v0.510.0
> **Status**: v0.5+ Phase 1+3 complete + Phase 2 L3 step 2 partial + §20 iterative audit (4 rounds). Writeback phases 10→7. §20 audit chain: Stage 18.412 (Shl/Shr lhs check) → 18.416 (BitAnd/BitOr/BitXor is_notable_ty check) → 18.420 (field access syntax mismatch) → 18.422 (&str indexing rejection + as_bytes Cast fix). All same-class soundness bugs (silent acceptance of invalid operations / design divergence from Rust). **ALL P0/P1/P2 TDs RESOLVED.** Only BLOCKED TDs require v0.5+ architecture work: TD-INTRINSIC-OVERUSE Phase 2-B/C, TD-STUB-PRELUDE-LOOP-BODY. 4506 tests (682 lib + 3824 integration), 0 failures (single-thread, ulimit -s unlimited). fmt clean, 0 clippy warnings. v0.4 release-ready.
