# Landin 编译器技术债完整清单 — v0.648.0 (Stage 116)

> **更新日期**: 2026-09-05
> **版本**: v0.648.0
> **状态**: v0.12 TD 修复阶段. Stage 116: LLVMShutdown() in Drop (reset LLVM C++ ManagedStatic). Debug impl re-add attempted + REVERTED — LLVM C++ heap allocator DenseMap pointer-address hashing still non-deterministic (0-5 failures per run). 新发现 TD-PROCESS-PER-TEST-ISOLATION (P3, v0.13+) — each compile() in a separate subprocess (like rustc).
...
| TD ID | 描述 | 根因 | 修复方案 | 依赖 |
|-------|------|------|---------|------|
| TD-TYPECK-WRITEBACK-INCOMPLETE | typeck Phase 3 不写 Constant.ty (留 Infer) + 部分泛型 def body 仍 emit Param. Stage 105 RCA: 100 次跑非确定性 SIGSEGV (3/100 ASLR on, 1/100 ASLR off). LLVM IR 在成功/失败跑间完全相同 (Param=73 Infer=18). 崩溃在 LLVM codegen/object emission 阶段. **Stage 110 完成 Phase 3.6 重新引入 — Constant type writeback, Infer warnings 41→19 (-54%)**. 残留 Param warnings from generic prelude def bodies (Vec::push<T> etc.) — TD-MONO-INFER 跟踪. | (1) typeck Phase 3 不写 Constant.ty → lit_to_const 创建 Infer(IntVar) ✅ Stage 110 修复 (Phase 3.6); (2) codegen Stage 14.64 src_ty 用 ConstVal (value size) 而非 c.ty → 不必要 sext cast ✅ Stage 109 修复; (3) TextEmitter emit_const_typed 返回 typed literal `"i64 0"` 与 consumer 双前缀冲突 ✅ Stage 109 修复; (4) 非 turbofish path generic call FnDef substs 未推断 (TD-MONO-INFER); (5) LLVM module 全局状态累积 (TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION). | Stage 111: 加 Debug impl 验证 100 次跑 0 SIGSEGV — Phase 3.6 active 后 c.ty 全部 concrete, 验证非确定性 SIGSEGV 是否消除. Stage 112+: TD-MONO-INFER (非 turbofish path generic substs) + TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION (LLVM module state). | TD-CODEGEN-CALL-ARG-TYPE-SOURCE ✅ (Stage 107) + TD-CODEGEN-CONST-SRC-TY-FROM-CONSTVAL ✅ (Stage 109) + Phase 3.6 ✅ (Stage 110) |
| TD-PRELUDE-IMPL-BODY-CODEGEN-CRASH (Layer 2-3) | prelude impl method bodies 在 LLVM integration tests 中触发非确定 SIGSEGV/SIGABRT (signal 11/6)。User code 中相同结构 impl method returning String/struct 工作正常 (`test_sret2.landin → 42`)。导致无法添加 Debug/PartialOrd impls。**Layer 1 已修复 (Stage 100)**, **Layer 2 部分修复 (Stage 101 — turbofish path)**, **Layer 4 已修复 (Stage 102 — Drop 释放 context)**, 剩余 Layer 2 (非 turbofish path, TD-MONO-INFER) + Layer 3 (TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION). | **4-layer 根因链** (Stage 99 RCA): (1) ✅ Layer 1 prelude generic methods 的 Param type 未解析 — Stage 100 修复 (跳过未实例化的 prelude generic, Param warnings 1360→24 -98%); (2) ⚠️ Layer 2 `mir_type_to_emit_type` 对 Param/Never fallback 到 i32 产生不正确 LLVM IR — Stage 101 部分修复 (turbofish path FnDef substs mangling, 非 turbofish path 待 TD-MONO-INFER); (3) ⚠️ Layer 3 加 Debug impl 后 LLVM module 全局变量累积触发 verify/emit crash — Stage 102 加 Debug impl 仍触发 14 失败, 需 Stage 103+ 调查 (TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION); (4) ✅ Layer 4 `LLVMSysEmitter::Drop` 不释放 context 加速累积 — Stage 102 修复 (Drop 释放 module + context). 详见 `docs/develop/v0/stage-99/dev-log.md`. | **4-stage 渐进重构**: ✅ Stage 100 Layer 1 完成; ⚠️ Stage 101 Layer 2 部分完成 (turbofish path); ✅ Stage 102 Layer 4 完成; Stage 103 调查 TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION (Layer 3); Stage 104 重新添加 Debug + PartialOrd impls; TD-MONO-INFER (P3, v0.11+) 完全消除 Param warnings | TD-STRUCT-RETURN-FROM-PRELUDE-IMPL-CODEGEN-CRASH ✅ (Stage 98) |
| TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION | 加 Debug impl 到 prelude 后 cargo test 14 个失败 (Stage 99 RCA). Layer 4 (Drop 不释放 context) 已修复但仍触发. **Stage 111 RCA 确认**: 即使 Layer 1+2+4 + Phase 3.6 (Stage 110) 全部修复, 加 Debug impl bodies 仍触发 10-18/4755 非确定性 SIGSEGV (3 次跑不同失败集). **Stage 114 RCA 确认**: 即使 Stage 113 修复了 TD-MONO-INFER + TD-LLVM-OBJ-EMIT-CRASH, 加 Debug impl bodies 仍触发 9-23/4788 非确定性 SIGSEGV. 根因: Debug impl bodies add vtable + dynptr globals per type → pushes LLVM module global count past crash threshold → LLVM CodeGenLevelDefault optimizer non-deterministically crashes. **Stage 114 仍为 Debug impl re-add 硬阻断**. | LLVM module 全局状态在 cargo test 多次 compile() 间累积. Drop 释放 context 不够 — module 本身的全局状态 (vtable/dynptr globals + function defs) 仍在 cargo test 多次 compile() 间累积. Debug impl bodies 是 concrete impl methods (非 generic), Stage 113 skip rule 不适用. | 1) 隔离每次 compile() 的 LLVM module state — 每次创建独立 LLVMContext (已实现); 2) 减少全局状态依赖 — prelude impl body 触发的 vtable/dynptr globals 在 module 间共享; 3) 考虑 LLVM 22 的 `LLVMRustExecutionContext` (LLVM 19+ per-thread context); 4) 参考 rustc 的 LLVM binding (rustc_llvm) 如何处理 module state. | TD-PRELUDE-IMPL-BODY-CODEGEN-CRASH Layer 4 ✅ (Stage 102) + TD-MONO-INFER ✅ (Stage 113) + TD-LLVM-OBJ-EMIT-CRASH ✅ (Stage 113) |
| TD-MONO-INFER | 非 turbofish path 的 generic call (e.g., `Box::new(42i32)`) 在 MIR lower 时 FnDef substs 为空。导致 codegen_operand 无法 mangle 到实例化名, generic def body 必须仍 emit (产生 19 Param warnings, Stage 110 baseline). **Stage 111 RCA 确认**: 这 19 Param warnings + Debug impl bodies 一起触发 LLVM module 全局状态累积 → 非确定性 SIGSEGV. **Stage 111 升级为 Debug impl re-add 硬阻断**. **Stage 112 RCA**: 尝试 fix (codegen skip rule + writeback secondary pass) 导致 43 linker errors (fix #1 alone) 或 6 impl Trait --emit-obj crashes (fix #1+#2). 新发现 TD-LLVM-OBJ-EMIT-CRASH 阻断 Stage 112 fix. | `lower_path_generic_args` 只看 turbofish (`<i32>`), 对 inference 推断的 substs 不填充. typeck 未反向传播 inferred substs 到 FnDef 类型的 call sites. | 在 typeck 完成后, 反向传播 inferred substs 到 FnDef 类型的 call sites. 参考 rustc `InferCtxt` + `TypeVariable` 设计. writeback_fndef_substs 已实现 (driver/compile_inner.rs:1017), 但需要扩展为 non-turbofish path + secondary pass for Assign's Operand::Constant. **Stage 112 fix blocked by TD-LLVM-OBJ-EMIT-CRASH**. | TD-PRELUDE-IMPL-BODY-CODEGEN-CRASH (Layer 2) ✅ partial (Stage 101) + Debug impl re-add (Stage 111 验证依赖) + TD-LLVM-OBJ-EMIT-CRASH (Stage 112 新发现) |
| TD-LLVM-OBJ-EMIT-CRASH | 确定性 SIGSEGV in `LLVMTargetMachineEmitToFile` when emitting object files for IR with `Operand::Constant` FnDef types that have non-empty inferred substs (from writeback_fndef_substs secondary pass). The IR is valid (llvm-as + llc both work), but the in-memory LLVMModule crashes when emitted via the C API. **Stage 112 新发现**: 阻断 TD-MONO-INFER fix (Stage 112 fix #1+#2 一起导致 6 impl Trait --emit-obj crashes). **Stage 113 修复**: build_fn_sigs_map 添加 specialized function sigs → 正确 forward declaration → 无 type mismatch. | Likely use-after-free or module state issue in `LLVMSysEmitter::emit_to_object_file` or similar LLVM C API binding path. The crash happens AFTER IR generation (IR is valid) but DURING object file emission. Stage 102 fixed Drop to release module+context, but the emit path may have a separate issue. **Stage 113 root cause**: build_fn_sigs_map 缺少 specialized function sigs → variadic forward decl → type mismatch → dangling references. | 1) 用 lldb/valgrind 调试 `LLVMSysEmitter::emit_to_object_file` 找到 crash site; 2) 检查 LLVMModule 在 emit 期间是否被释放 (use-after-free); 3) 考虑 LLVM 22 的 `LLVMRustExecutionContext` (LLVM 19+ per-thread context) 作为隔离方案; 4) 参考 rustc 的 LLVM binding (rustc_llvm) 如何处理 object emission. **Stage 113 fix**: build_fn_sigs_map 添加 specialized sigs. | TD-MONO-INFER (Stage 112 fix blocked by this TD) |
| TD-TRAIT-METHOD-AMBIGUITY | 当两个 trait 都有同名 method (e.g., Display::fmt + Debug::fmt), method resolution 选 Display (wrong) 而非 Debug (correct). `n.fmt()` 调用 Display::fmt (returns i64) 而非 Debug::fmt (returns String) → type mismatch error. **Stage 114 新发现**: 阻断 Debug impl usability (即使 SIGSEGV 修复, Debug::fmt 仍无法正确调用). | trait method resolution 没有基于 return type 或 expected type 区分同名 method. Rust 用 explicit trait dispatch (`<i32 as Debug>::fmt(&n)`) 或 UFCS 解决, Landin 尚未支持. | 1) 添加 explicit trait dispatch 语法 (`<i32 as Debug>::fmt(&n)`); 2) method resolution 基于 expected return type (如果 expected String, 选 Debug::fmt); 3) 重命名 Debug::fmt 为 Debug::debug_fmt (避免与 Display::fmt 冲突). | None (Stage 114 新发现) |
| TD-LLVM-INTERNAL-NONDETERMINISM | LLVM's C++ code uses hash-based data structures (SelectionDAG, register allocator, instruction scheduler) that depend on memory layout. Even with deterministic IR (from Stage 115 sort fixes) + LLVMShutdown() (Stage 116), LLVM's backend can produce different machine code between runs, occasionally crashing (0-5 non-deterministic SIGSEGV per cargo test run with Debug impl bodies). **Stage 116 新发现**: 阻断 Debug impl re-add (0-5 > 0, §3.2 red line). | LLVM's C++ `DenseMap`/`MapVector`/`SetVector` use pointer-based hashing that varies between runs due to ASLR + memory layout. The `SelectionDAG` ISel pipeline and `RegAlloc` are the primary sources. LLVMShutdown() resets ManagedStatic but NOT the C++ heap allocator. | 1) Use `LLVMRustExecutionContext` (LLVM 19+ per-thread context isolation); 2) Run each cargo test in a separate process (process-per-test isolation); 3) Fork LLVM to fix C++ hash table non-determinism (not practical); 4) Accept 0-5 failure rate for Debug impls (violates §3.2). | TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION ✅ partial (Stage 115 sort fixes) |
| TD-PROCESS-PER-TEST-ISOLATION | cargo test invokes `compile()` multiple times in the same process. LLVM C++ global state (heap allocator, DenseMap hash tables) accumulates, producing non-deterministic backend behavior. **Stage 116 新发现**: The only complete fix for TD-LLVM-INTERNAL-NONDETERMINISM is process-per-test isolation (each compile() in a separate subprocess, like rustc). | Single-process model: all compile() calls share LLVM C++ state. rustc uses process-per-compilation model — each rustc invocation is a separate process. | 1) Wrap `compile()` in `std::process::Command` — fork a child process that compiles and returns the result; 2) Use `std::os::unix::process::CommandExt` for efficient fork; 3) Consider `LLVMRustExecutionContext` as an alternative (LLVM 19+ per-thread context, but doesn't isolate heap allocator). | TD-LLVM-INTERNAL-NONDETERMINISM (Stage 116 confirmed as the only complete fix) |
