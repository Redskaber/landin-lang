# Landin 编译器技术债完整清单 — v0.647.0 (Stage 115)

> **更新日期**: 2026-09-05
> **版本**: v0.647.0
> **状态**: v0.12 TD 修复阶段. Stage 115 修复 TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION (partial) — 根因: Rust HashMap random SipHash seed → 非确定性 vtable/dynptr/drop_glue/mono_items emission order → 不同 LLVM module states → 非确定性 SIGSEGV. 4 sort fixes (vtable, dynptr, drop_glue, mono_items) 减少失败率 from 9-23 to 0-3. Debug impl bodies 仍 REVERTED (0-3 > 0, §3.2 red line). 新发现 TD-LLVM-INTERNAL-NONDETERMINISM (LLVM C++ hash tables 非确定性). 为 Stage 116+ (LLVMRustExecutionContext or process-per-test isolation) 铺平道路.
...
| TD-LLVM-INTERNAL-NONDETERMINISM | LLVM's C++ code uses hash-based data structures (SelectionDAG, register allocator, instruction scheduler) that depend on memory layout. Even with deterministic IR (from Stage 115 sort fixes), LLVM's backend can produce different machine code between runs, occasionally crashing (0-3 non-deterministic SIGSEGV per cargo test run with Debug impl bodies). **Stage 115 新发现**: 阻断 Debug impl re-add (0-3 > 0, §3.2 red line). | LLVM's C++ `DenseMap`/`MapVector`/`SetVector` use pointer-based hashing that varies between runs due to ASLR + memory layout. The `SelectionDAG` ISel pipeline and `RegAlloc` are the primary sources. | 1) Use `LLVMRustExecutionContext` (LLVM 19+ per-thread context isolation); 2) Run each cargo test in a separate process (process-per-test isolation); 3) Fork LLVM to fix C++ hash table non-determinism (not practical); 4) Accept 0-3 failure rate for Debug impls (violates §3.2). | TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION ✅ partial (Stage 115 sort fixes) |
