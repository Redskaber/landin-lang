# Landin 编译器技术债完整清单 — v0.645.0 (Stage 110)

> **更新日期**: 2026-09-05
> **版本**: v0.645.0
> **状态**: v0.12 TD 修复阶段. Stage 110 完成 Phase 3.6 (Constant type writeback) 重新引入 — typeck Phase 3.6 walks all Operand::Constant in statements + terminators and writes unify.resolve(&c.ty) back to c.ty. Infer warnings 41→19 (-54%) on representative sample. 残留: TD-MONO-INFER (非 turbofish path generic FnDef substs) + TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION (LLVM module 全局状态累积). Stage 110 重新引入 Phase 3.6 无回归, 为 Stage 111 (加 Debug impl 验证 100 次跑 0 SIGSEGV) 铺平道路.
