...
| TD-STUB-TYPECK-BEFORE-WRITEBACK | typeck 在 writeback 之前运行, 导致 local_decl.ty 可能含未替换 Param. | `src/driver/compile_inner.rs:633 (typeck) vs 881 (writeback)` | driver order | ✅ Partial Stage 18.353: 在 typeck `check_mir_body_with_tables` 的 Phase 1 之前添加 Phase 0 pre-writeback — 调用 `writeback_type_propagation(mir, &self.fn_sigs)` 让 local_decls 中的 Param 在 typeck 看到之前被 Rule 3 Field projection 解析. writeback 是幂等的, 重复运行不破坏 (driver 在 typeck 后再运行一次, 结果一致). Per §1.0 原則 6 (通解 > 特解): 一个 pre-writeback pass 覆盖所有 Param 泄漏路径. Per §1.0 原則 9 (正确 > 妥协): Phase 0 比 driver 重排风险低. Per §12 (最优 > 最小): 根因修复在 typeck 边界. **已知限制**: Holder<T> { ptr: *mut T } field access 仍报 false error — 根因在 lower 层 let-binding struct literal adt_substs 推断 (h 的 local_decl.ty 在 lower 时 substs 为空). Stage 18.346 部分修复了 Aggregate path, 但 let-binding path 仍有 gap. v0.5+ 修复 expected_ty 在 let-binding 中的传播. |
