# Landin 编译器技术债完整清单 — v0.645.0 (Stage 112 RCA)

> **更新日期**: 2026-09-05
> **版本**: v0.645.0 (无版本变更 — Stage 112 RCA + revert)
> **状态**: v0.12 TD 修复阶段. Stage 112 TD-MONO-INFER fix attempted + REVERTED — two-part fix (codegen skip rule + writeback secondary pass) caused 43 linker errors (fix #1 alone) or 6 impl Trait --emit-obj crashes (fix #1+#2). 新发现 TD-LLVM-OBJ-EMIT-CRASH (确定性 SIGSEGV in LLVMTargetMachineEmitToFile). 阻断 TD-MONO-INFER fix. Stage 111 baseline preserved (Phase 3.6 active, Debug impl deferred, 5663 tests). 为 Stage 113 (TD-LLVM-OBJ-EMIT-CRASH 调查) + Stage 114 (TD-MONO-INFER fix retry) + Stage 115 (Debug impl re-add) 铺平道路.
