# Landin 编译器技术债完整清单 — v0.644.0 (Stage 109)

> **更新日期**: 2026-09-05
> **版本**: v0.644.0
> **状态**: v0.11 TD-PRELUDE-IMPL-BODY-CODEGEN-CRASH 修复阶段 (Layer 1+2+3+4 部分完成)。Stage 109 完成 TD-CODEGEN-CONST-SRC-TY-FROM-CONSTVAL 修复 — codegen operand.rs 当 c.ty 为 concrete Int/Uint/Bool/Char 时用 emit_const_typed 直接 emit (跳过 sext/trunc cast); 同时修复 Stage 18.287 遗留 bug — TextEmitter emit_const_typed 返回 raw value (无 type prefix), 与 LLVM emitter contract 对齐。残留: TD-TYPECK-WRITEBACK-INCOMPLETE — Phase 3.6 (Constant type writeback) 待 Stage 110 重新引入 (Stage 109 已修复前置依赖).
...
| TD ID | 描述 | 根因 | 修复方案 | 依赖 |
|-------|------|------|---------|------|
| TD-TYPECK-WRITEBACK-INCOMPLETE | typeck Phase 3 不写 Constant.ty (留 Infer) + 部分泛型 def body 仍 emit Param. Stage 105 RCA: 100 次跑非确定性 SIGSEGV (3/100 ASLR on, 1/100 ASLR off). LLVM IR 在成功/失败跑间完全相同 (Param=73 Infer=18). 崩溃在 LLVM codegen/object emission 阶段. Stage 106 尝试 Phase 3.6 (Constant type writeback) → 7 回归. Stage 107 修复 TD-CODEGEN-CALL-ARG-TYPE-SOURCE. Stage 108 再次尝试 Phase 3.6 → 7 回归 (TD-CODEGEN-CONST-SRC-TY-FROM-CONSTVAL). **Stage 109 修复 TD-CODEGEN-CONST-SRC-TY-FROM-CONSTVAL — codegen src_ty 用 c.ty**. | (1) typeck Phase 3 不写 Constant.ty → lit_to_const 创建 Infer(IntVar); (2) codegen Stage 14.64 src_ty 用 ConstVal (value size) 而非 c.ty → 不必要 sext cast ✅ Stage 109 修复; (3) TextEmitter emit_const_typed 返回 typed literal `"i64 0"` 与 consumer 双前缀冲突 ✅ Stage 109 修复; (4) 非 turbofish path generic call FnDef substs 未推断 (TD-MONO-INFER); (5) LLVM module 全局状态累积 (TD-PRELUDE-IMPL-BODY-MODULE-ACCUMULATION). | Stage 110: 重新引入 Phase 3.6 (Constant type writeback) — Stage 109 已修复前置依赖 (codegen src_ty + TextEmitter contract). Stage 111: 加 Debug impl 验证 100 次跑 0 SIGSEGV. | TD-CODEGEN-CALL-ARG-TYPE-SOURCE ✅ (Stage 107) + TD-CODEGEN-CONST-SRC-TY-FROM-CONSTVAL ✅ (Stage 109) |
