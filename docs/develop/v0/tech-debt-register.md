...
> **Author**: redskaber
> **Date**: 2026-08-25 (last updated Stage 18.260 — Phase 2d-2f gap analysis)
> **Version**: v0.492.0
> **Status**: Active — all P0/P1 items resolved. TD-TUPLE-CTOR-TYPECK Phase 1+2a+2b+2c RESOLVED (Stages 18.255-18.258). TD-UNIFY-ARG-ORDER (P3) RESOLVED Stage 18.259. Stage 18.260 gap analysis identified remaining narrow gap (Phase 2e — call arg path) documented as MVP, deferred to v0.3+ when fn_sigs access in MIR lower becomes feasible. Remaining: TD-INTRINSIC-OVERUSE Phase 2 (blocked on v0.4+ language features), TD-DROP-MOVED-LOCALS full (deferred to v0.3+ flow-sensitive tracking).
...
| TD-TUPLE-CTOR-CALL-ARG | When generic tuple struct ctor is passed as function arg (e.g., `take_holder(Holder(true))` where `fn take_holder(h: Holder<i32>)`), soundness hole remains. typeck's unify table silently accepts `Adt(def, []) ↔ Adt(def, [i32])` because empty substs are treated as "unknown, to be inferred". | `take_holder(Holder(true))` does NOT error — soundness hole in narrow case (function/method call args). All other cases (let binding, return expr, if/else, match, array) are closed. | 🟡 NEW (Stage 18.260 gap analysis) — Phase 2e deferred. Root cause: MIR lower doesn't have `fn_sigs` access (built in writeback, AFTER MIR lower), so `expected_ty` from sig.inputs[i] can't be threaded into arg's `lower_expr_to_operand`. Fix options: (A) thread fn_sigs into MIR lower (architectural change — fn_sigs currently built in writeback); (B) typeck-level fix via unify table modification (requires generics_of access, violates §11); (C) pre-compute generic_adt_def_ids in resolver; (D) defer to v0.3+ when trait solver / GATs require fn_sigs access in MIR lower. Selected: Option D (defer). Rationale: gap is narrow (only call args of generic tuple struct ctors); all other cases closed; Options A/B/C require architectural changes touching multiple modules; v0.3+ will naturally require fn_sigs access. Per §17.6 documented as MVP with fix plan. Regression tests: `tests/v0/stage18/plan/stage18_260_phase2d_2f_gap_analysis_tests.rs` (5 tests: 4 closed + 1 deferred-MVP marker). |
