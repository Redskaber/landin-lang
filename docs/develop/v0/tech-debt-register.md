...
| TD-GENERIC-FN-RETURN-EXPECTED-TY | When generic tuple struct ctor with wrong type is used in fn body return position (e.g., `fn make() -> Holder<i32> { Holder(true) }`), soundness hole remains. Root cause: `expected_ty` from fn sig return type is not threaded into fn body's `lower_expr_to_operand` calls. | `fn make() -> Holder<i32> { Holder(true) }` does NOT error — soundness hole in fn body return path. | 🟡 NEW (Stage 18.268 audit) — Phase 2d deferred. Root cause: MIR lower doesn't have access to the fn's own return type when lowering the body (return_ty is computed by driver but not threaded into `lower_hir_body_to_mir_full_with_dyn_trait_plan` for expected_ty propagation). Fix requires threading `fn_return_ty: Option<&Ty>` into MIR lower as a 8th param (similar to fn_sigs in Stage 18.262). Per §17.6: same class as TD-TUPLE-CTOR-CALL-ARG. Per §1.0 原則 9 (正确 > 妥协): documented as MVP with fix plan. Rationale: gap is narrow (only fn body return position with generic tuple struct ctors); workaround is explicit turbofish (`Holder::<i32>(true)`); fix is straightforward but adds another param to entry point signature. |
