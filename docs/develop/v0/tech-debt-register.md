...
> **Author**: redskaber
> **Date**: 2026-08-28 (last updated Stage 18.349 — Typeck strictness investigation)
> **Version**: v0.510.0
> **Status**: Stage 18.349 investigated typeck strictness gaps. Found: (1) `let p: Pair = ...` (missing generic args) triggers TD-GENERIC-PARAM-CHECK returning Error type, but typeck doesn't report Error in local_decls — root cause is prelude TD-INTRINSIC-OVERUSE Phase 2-B/C BLOCKED (47 false-positives if check enabled). (2) `p.second = 100i32` (i64 field assigned i32) is actually valid Landin code — Stage 3.59 narrowing rule allows narrow→wide implicit conversion (design choice, not bug). Phase 4.5 check disabled until prelude fixed. **ALL P0/P1/P2 TDs RESOLVED.** Only BLOCKED TD: TD-INTRINSIC-OVERUSE Phase 2-B/C (needs lang features). 4395 tests (682 lib + 3713 integration), 0 failures (single-thread, ulimit -s unlimited). v0.4 release-ready.
