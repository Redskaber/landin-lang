...
> **Author**: redskaber
> **Version**: 8.0（Stage 162: 流程文档重构 — 补充 TD 触发规则、依赖限制处理、可疑代码模式审计）
...
| 交付前验收 | §3.2, §19 | cargo clean+build+check+fmt+clippy+test --release 全绿 + 打包 tar.gz |
...
flowchart TD
    ...
    Writeback --> CodeClean["§14.9 阶段末尾系统性代码整理<br/>C1-C6 六维度检查"]
    CodeClean --> CrossStage["§14.6 阶段间深度验证<br/>4 项 + 多轮深挖 [L3]"]
    CrossStage -->|GO| Next([下一阶段])
    CrossStage -->|NO-GO| Refactor[§13.4 重构治理六大判据]
    Refactor --> Inner
```

> **L1/L2 路径**：L1 任务只需 `Design → Plan → Inner → Q6 → Gate → Outer → Commit`（跳过 §14.5/§14.6/§14.8/§14.9）；L2 任务执行到 `Writeback` + `CodeClean` 即可（跳过 §14.6）。
...
- **阶段末尾系统性代码整理**（§14.9）：在 §14.5 深度审查和 §14.8 设计回写完成后，对整个项目进行系统性全面代码的整理——代码清洁、同类聚块、注释时效同步、文件头信息检查、API 命名规范检查、数据结构与流标准化处理。确保代码库在进入下一阶段前处于"干净基线"状态。
...
### 2.1.1 每轮执行原则（v7.0 新增）

> **目的**：确保每一轮 stage 执行都严格遵循设计、开发、测试原则，不因"赶进度"而跳过关键质量步骤。

每轮 stage 执行时，Agent 必须遵守以下原则：

| # | 原则 | 说明 | 违反示例 |
|---|------|------|---------|
| 1 | **按计划推进任务** | 根据当前审查报告、stage、plan、gate-review、复盘情况、开发记录，结合设计文档与项目实际情况，按照计划推进修复 | 跳过 plan 中列出的任务直接做下一个 |
| 2 | **严格 API 命名标准化** | 所有代码变更必须遵循 §10 API 命名标准 | 使用 glob re-export 或不一致命名 |
| 3 | **严格接口设计** | 所有接口变更必须遵循 §11 接口隔离原则 | codegen 直接调用 mir::lower 内部函数 |
| 4 | **通解 > 特解** | 优先实现通用机制，避免逐案特例处理（§2.2 原则 6） | 为每种 ADT 形态在 codegen 加单独 `if` 分支 |
| 5 | **高内聚低耦合** | 每个模块/文件承担且仅承担一个明确职责；模块间通过数据契约交互 | utils.rs 同时包含类型转换 + 错误格式化 + IO 助手 |
| 6 | **单一职责** | 每个模块、函数、类型只做一件事，用一句话能描述 | lower_expr 同时处理表达式降低 + 控制流 + 内置函数 |
| 7 | **管道流** | 编译流水线各阶段单向流动（lexer → parser → HIR → resolve → MIR → typeck → borrowck → codegen），不允许回流或回查 | codegen 反向查询 HIR 获取类型信息 |
...
| 14 | **功能缺失即时同步 TD** (v8.0) | 开发中发现功能缺失或当前存在限制时，**立即**将问题描述、影响范围、修复方案同步到 `tech-debt-register.md`，不得推迟到阶段末尾。同步在 `worklog.md` 中记录 TD ID 和规划。 | 发现类型推断不支持某模式但不记录 TD，继续用 workaround 推进 |
| 15 | **依赖限制阻断推进** (v8.0) | 开发中发现任务所需的依赖内容存在限制时，**停止继续做阉割版任务推进**（正确 > 妥协），转而分析缺失的依赖内容并同步规划到 tech-debt。可参考同类优秀实例（如 Rust）的相关设计进行修复补全。已实现的阉割版内容也必须将修复计划同步到 tech-debt。 | 发现闭包不支持但继续用函数指针 workaround 推进而不记录 TD；发现 `mangle_ty` 对 Error 类型返回 "error" 但不记录为限制 |
| 16 | **可疑代码模式审计** (v8.0) | 开发/审查时遇到以下模式必须**怀疑为 bug 或不完整实现**，纳入 tech-debt 规划：(1) 下划线开头的变量名未使用；(2) 空值/None 传递未记录原因；(3) 默认值/默认分支使用未说明；(4) 不完整 match arm（缺少覆盖）。每种情况必须判断"真的不需要"还是"预留漏洞"，并在 worklog 中注明判断依据。 | `let _unused = ...` 而不记录原因；`match` 缺少 `_ =>` arm 但不报 non-exhaustive；传递 `None` 给 `mono_layouts` 而不在 tech-debt 记录 |

### 2.2 核心设计决策原则

> **来源**：用户在 Stage 14 持续审查中反复强调的决策原则，贯穿所有代码、重构、测试、文档决策。这些原则不是建议而是硬性约束——任何 Agent 在面临设计选择时必须以此为准绳。

以下 11 条原则按优先级排列，冲突时上面的优先：
...
| 10 | **唯一可信数据源** | 每个数据实体（类型、布局、签名等）在编译管道中应有且仅有一个权威来源。其他模块通过引用获取，不得复制或自行推断。当多处存储同一数据时，必须明确谁是 source of truth，其余为 cache/view。 | `local_decl.ty` 是 local 类型的唯一可信源；`ProjectionElem::Field(_, field_ty)` 中的 `field_ty` 是 cache，必须与 `local_decl.ty` 一致。`find_receiver_substs` 签名是 `&cx` 时不能调用 `&mut cx` 函数修改 MIR — 必须尊重数据源的可变性边界。 |
| 11 | **确定性边界** | 在开始任何修复或设计前，必须先清晰划定能力边界（能做什么）、设计边界（架构允许什么）、职责边界（谁负责什么）。不确定的边界不得猜测，必须基于设计文档或 Rust 参考实现确定。 | 修复 `store ptr ptr null` 前，必须先划分 `emit_null_ptr()` 返回值（value only）与 `emit_store()` 类型前缀（type prefix）的职责边界 — 不能模糊两者。`find_receiver_substs(&cx)` 的能力边界是只读 MIR local_decls，不能 lower 新表达式 — 这是确定性边界。 |
...
- 原则 10（唯一可信数据源）是**数据架构基石**——确保数据一致性，防止多源冲突
- 原则 11（确定性边界）是**设计前置条件**——确保在动手前清晰边界，防止越界操作
...
**原则 10（唯一可信数据源）的执行要求**：
- 每个数据实体（类型、布局、签名、substs 等）在编译管道中应有且仅有一个**权威来源**
- 其他模块通过**引用**获取数据，不得**复制**或**自行推断**
- 当多处存储同一数据时，必须明确谁是 **source of truth**，其余为 **cache/view**
- Cache 必须与 source of truth 保持一致 — 如果 source 被修改，cache 必须同步更新（或标记为 stale）
- 函数签名（`&T` vs `&mut T`）决定了对数据的可操作性 — `&cx` 只能读，`&mut cx` 可读写
- 当函数签名限制了对数据的访问时（如 `&cx` 不能调用 `&mut cx` 的 `lower_expr_to_operand`），不得通过 hack 绕过 — 必须尊重数据源的可变性边界
- 参考 Rust rustc 的 `TyCtxt` — 它是所有类型查询的唯一入口，通过 `&self` 提供只读访问

**原则 11（确定性边界）的执行要求**：
- 在开始任何修复或设计前，必须先清晰划定三类边界：
  1. **能力边界**（Capability Boundary）：当前模块/函数**能做什么** — 由其签名、数据访问权、依赖决定
  2. **设计边界**（Design Boundary）：当前架构**允许什么** — 由设计文档、§16 接口隔离、§13.4 重构判据决定
  3. **职责边界**（Responsibility Boundary）：当前模块**负责什么** — 由 §16 接口隔离、单一职责原则决定
- 不确定的边界不得猜测 — 必须基于设计文档或 Rust 参考实现确定
- 当边界冲突时（如需要 `&mut cx` 但签名是 `&cx`），必须先解决边界冲突（修改签名 or 换路径），不得 hack 绕过
- 参考 Rust rustc 的 `InferCtxt` — 它的边界由 `&self` 签名明确划定，调用者不能越界修改
...
| `llvm-config` + `llvm-sys` | LLVM 22 后端 | `source scripts/setup-llvm-env.sh` | `docs/llvm/` |
| `llc` / `opt` / `lld` | LLVM IR 验证+链接 | 随 LLVM 22 安装 | `docs/llvm/` |
...
**验收命令**（Landin 项目，v7.0 修正为 `--release`）：
```bash
cargo clean && cargo build --release --features llvm-backend && cargo check --features llvm-backend && cargo fmt --check && cargo clippy --all-targets --features llvm-backend -- -D warnings && cargo test --release --features llvm-backend
```
...
| `cargo build --release --features llvm-backend` | 编译成功（release 优化模式） | 修复编译错误，不交付 |
| `cargo check --features llvm-backend` | 0 errors, 0 warnings (快速类型检查) | 修复类型错误/unused 警告，不交付 |
| `cargo test --release --features llvm-backend` | `0 failed`（ignored 可接受，release 优化模式） | 修复代码或更新测试，不交付 |
| `cargo fmt --check` | exit 0（零 diff） | 运行 `cargo fmt` 修复 |
| `cargo clippy --all-targets --features llvm-backend -- -D warnings` | `0 warnings` | 修复 lint，不交付 |

**流程**：
1. 完成代码+文档改动
2. 运行 `cargo clean`
3. 运行 `cargo build --release --features llvm-backend` — 必须成功
4. 运行 `cargo check --features llvm-backend` — 必须 0 errors + 0 warnings（快速类型检查，捕获 unused_mut 等）
5. 运行 `cargo test --release --features llvm-backend` — 必须全绿
6. 运行 `cargo fmt`（apply）+ `cargo fmt --check`（验证）
7. 运行 `cargo clippy --all-targets --features llvm-backend -- -D warnings` — 必须 0 warnings
8. 全绿后按 §19 规则打包 tar.gz 并返回
...
>
> **状态**：§3.5 规划中——auto-query / auto-install / auto-configure 尚未实现。当前使用 `scripts/setup-llvm-env.sh` + `scripts/switch-llvm-version.sh` 作为手动工具链。未来实现后归档到 `tools/dev/`。

**三个规划工具**（实现后归档到 `tools/dev/`）：
...
3. `cargo build --release --features llvm-backend` 0 warnings。
4. `cargo clippy --all-targets --features llvm-backend -- -D warnings` 通过。
...
#### 6.2.0 技术债触发时机与处理流程（v8.0 新增）

> **目的**：明确在开发流程中何时必须同步 TD 到技术债队列，以及如何处理依赖限制。

**触发时机 1：功能缺失或当前存在限制**

| 触发条件 | 处理流程 | 示例 |
|---------|---------|------|
| 开发中发现功能缺失（语言特性不支持） | (1) 停止当前任务推进 (2) 分析缺失功能的影响范围 (3) 同步到 `tech-debt-register.md` 含描述/根因/修复方案/优先级 (4) 在 `worklog.md` 记录 TD ID (5) 如果有 workaround，在 TD 中记录 workaround 及其限制 | 发现闭包不支持 → 记录 TD-FN-CLOSURE-COERCION，用函数指针 workaround 并记录限制 |
| 开发中发现当前实现有限制 | 同上 | `mangle_ty` 对 Error 类型返回 "error" → 记录 TD-INFERRED-TYPE-METHOD-MANGLING |

**触发时机 2：依赖内容存在限制**

| 触发条件 | 处理流程 | 关键原则 |
|---------|---------|---------|
| 任务所需的依赖功能不存在或被限制 | (1) **停止继续做阉割版任务推进** (2) 分析缺失的依赖内容 (3) 同步规划到 `tech-debt-register.md` (4) 可参考同类优秀实例（如 Rust）的相关设计进行修复补全 (5) 如果存在已实现的阉割版内容，将完整修复计划同步到 tech-debt | **正确 > 妥协**：不得以"后续阶段处理"为由继续推送阉割版。已实现的阉割版必须记录修复计划。 |

**触发时机 3：可疑代码模式（v8.0 新增）**

| 模式 | 怀疑理由 | 处理要求 |
|------|---------|---------|
| 下划线开头的变量名未使用（`_unused`） | 可能是预留漏洞 — 真的不需要还是设计不完整？ | (1) 判断是否真的不需要 (2) 如果不确定，纳入 tech-debt 规划 (3) 在 worklog 注明判断依据 |
| 空值/None 传递未记录原因 | 可能是临时桩 — 真实设计还是不完整实现？ | (1) 检查是否有文档说明 (2) 如无说明，在 tech-debt 记录 (3) 评估是否影响正确性 |
| 默认值/默认分支使用未说明 | 可能是静默降级 — 正确处理还是掩盖 bug？ | (1) 检查默认值是否合理 (2) 如不合理，记录为 TD (3) 评估是否有报错路径替代 |
| 不完整 match arm（缺少覆盖） | 可能是非穷尽匹配 — 有意为之还是遗漏？ | (1) 检查是否有 `_ =>` catch-all (2) 如无 catch-all 且类型有更多 variant，记录为 TD (3) 评估是否会导致 panic |
...
flowchart TD
    ...
    F -->|不通过| D
    G --> H["7. 验收 §3.2<br/>cargo clean+build --release+check+fmt+clippy+test --release 全绿"]
...
> **目的**：当处于一个阶段的末尾时（处理时机：§14.5 深度审查通过后、§14.8 设计回写完成后、阶段切换前），必须对整个项目进行系统性全面代码的整理。这不是可选的"美化"——而是确保代码库在进入下一阶段前处于"干净基线"状态的强制协议。
>
> **核心理念**：阶段切换是代码库"熵减"的最佳时机。一个阶段内积累的技术债（注释过时、命名不一致、文件头缺失、同类代码分散）如果不在此节点清理，会在下一阶段被新代码"继承"和"扩散"。本协议把"系统性代码整理"从可选行为固化为强制行为。
>
> **触发时机**：
> 1. §14.5 深度审查通过后
> 2. §14.8 设计回写完成后
> 3. 阶段切换前（§13.2 切换期）
>
> **执行者**：DEV-A 主导执行，REV-A 审查整理质量，PM-A 确认完成。

#### 14.9.1 整理维度（6 项强制检查）

| # | 维度 | 检查内容 | 验证方法 |
|---|------|---------|---------|
| **C1** | **代码清洁** | 删除死代码（unused imports/functions/variables）、删除被注释掉的代码块、删除 TODO/FIXME 中已完成的项 | `cargo build --features llvm-backend` 0 warnings + `cargo clippy -- -D warnings` 0 warnings |
| **C2** | **同类聚块** | 同类功能代码聚集在同一模块/文件内，不分散在多处。同一抽象层级的代码在同一目录。跨模块引用通过 `pub use` 显式导出（§5.1），不通过 glob re-export（§10） | 人工审查目录结构 + `grep -r "pub use"` 确认无 `*` 通配符 |
| **C3** | **注释时效同步** | 所有 `// Stage N` 注释中的 Stage 编号与当前版本一致。设计文档引用的章节编号准确。删除引用已删除代码的注释。更新因重构而失效的注释 | `grep -rn "Stage [0-9]" src/` 人工审查 + `grep -rn "TODO\|FIXME\|HACK\|XXX" src/` 确认每个项仍有效 |
| **C4** | **文件头信息检查** | 每个模块的 `//!` 文件头注释准确描述当前职责（不是过时的旧描述）。`Stage N` 标记与实际版本一致。文件头不描述已删除的功能 | 逐文件人工审查 `//!` 文件头 |
| **C5** | **API 命名规范检查** | 所有 `pub fn` / `pub struct` / `pub enum` 遵循 §10 命名规则（`<verb>_<noun>` / `UpperCamelCase` / `UPPER_SNAKE_CASE`）。无缩写不一致（如 `fmt` vs `format` 混用）。无 Hungarian notation 残留 | `grep -rn "pub fn\|pub struct\|pub enum\|pub type" src/` 人工审查 |
| **C6** | **数据结构与流标准化** | 数据结构定义在正确的模块（§11 接口隔离）。数据流向清晰：driver → HIR → MIR → codegen，无逆向依赖。`CompileResult` 字段命名一致。`pub` 可见性最小化（无不必要的 `pub`） | 人工审查 `src/` 目录结构 + `grep -rn "pub " src/ | wc -l` 确认无过度暴露 |

#### 14.9.2 执行流程

```mermaid
flowchart TD
    A["§14.5 深度审查通过"] --> B["§14.8 设计回写完成"]
    B --> C["C1 代码清洁<br/>删除死代码 + 0 warnings"]
    C --> D["C2 同类聚块<br/>同类代码聚集 + 无 glob re-export"]
    D --> E["C3 注释时效同步<br/>Stage 编号准确 + 无过时 TODO"]
    E --> F["C4 文件头信息检查<br/>//! 描述准确"]
    F --> G["C5 API 命名规范检查<br/>§10 合规"]
    G --> H["C6 数据结构与流标准化<br/>§11 接口隔离 + pub 最小化"]
    H --> I["§3.2 验收全绿<br/>fmt + clippy + test --release"]
    I --> J["阶段切换就绪"]
```
...
系统性代码整理完成需满足：
1. 6 项检查全部完成（C1-C6）
2. `cargo fmt --check` 0 diff
3. `cargo clippy --all-targets --features llvm-backend -- -D warnings` 0 warnings
4. `cargo test --release --features llvm-backend` 全绿
5. 无死代码（`grep -rn "TODO\|FIXME\|HACK\|XXX" src/` 中每项都有明确理由或计划）
6. 无 glob re-export（`grep -rn "pub use.*::\*" src/` 结果为空或每项有明确理由）
7. 整理报告（简要）记录在 `docs/worklog.md` 的阶段末尾段落中

#### 14.9.4 与 §14.5 / §14.8 的关系

```
§14.5 深度审查（架构层面：设计是否足够？）
  ↓
§14.8 设计回写（文档层面：设计 vs 实现偏差？）
  ↓
§14.9 代码整理（代码层面：代码是否干净？）  ← 新增
  ↓
§3.2 验收全绿 → 阶段切换就绪
```

§14.5 审查"架构是否足够支撑下一阶段"。
§14.8 审查"设计文档与实现是否一致"。
§14.9 审查"代码本身是否干净、一致、可维护"。

三者互补：§14.5 关注架构层，§14.8 关注文档层，§14.9 关注代码层。任一环节缺失都会导致"架构对、文档对、但代码脏"的隐患——脏代码会在下一阶段扩散，增加维护成本。
...
| 版本 | 阶段 | 关键变更 |
| :--- | :--- | :--- |
| v1.0–v4.0 | Stage 1.1–14.114 | 初始流程 → 集成验证 → 文档同步 → API 命名 → 接口隔离 → 深度审查 → 项目图 → 重构治理 → 任务规划。全部特性已融入 §1-§17 当前结构。（v3.3 精要化：合并历史条目） |
| **v5.0** | **Stage 16.75** | 重构 v4.0：100% 覆盖原版意图，精简表达。合并冗余 changelog、移除历史覆盖矩阵、用表格替代段落。 |
| **v6.0** | **Stage 18.120** | 新增 §17 任务规划排版图（七步流程）。 |
| **v6.1–v6.4** | **Stage 18.122–18.125** | 深度审查修复：文件名修正、交叉引用修复、mermaid 标签修复、L1/L2/L3 分层、tech-debt-register + calibration-data 定义。 |
| **v7.0** | **Stage 18.275** | 整合用户 7 条执行指令 + 新增 §18（依赖审查）+ §19（阶段打包）+ §20（迭代审计）+ §2.1.1（每轮执行原则）+ §3.2 `--release` 修正 + §17.7 整体性修复 + §17.8 任务重排。 |
...
| 整体性修复 | 同类型错误或存在依赖关系的情况应考虑整体性完整修复，避免存在缺漏和遗失（v7.0 新增） |
| 适用范围 | 此条例适用于：设计、开发、测试、审查、修复优化重构、复写更新等所有流程 |

### 17.8 Step 7: 优化补充 — 审查规划图缺陷 + 任务重排
...
| **任务阻塞审查** | 如果任务依赖缺陷或环境阻塞，是否已做任务审查？（v7.0 新增） |
| **任务重排** | 如果当前能力不足以处理任务，是否已重排任务排版图（当前及以后节点阶段）？（v7.0 新增） |

**任务阻塞审查**（v7.0 新增）：

当开始选择处理任务时遇到任务依赖缺陷及环境等任务阻塞问题时：
1. **先做任务审查**：当前项目的能力是否具备处理当前任务？是否为最佳处理时机？
2. **如果不能**：重构重排任务排版图（当前及以后节点阶段的图）
3. **记录原因**：将重排原因及描述记录到开发文档中，避免不合理的任务排期和构建
...
## 18. 依赖与基础设施审查（v7.0 新增）

> **目的**：当设计内容需要依赖底层实现和功能时，必须先做依赖与基础设施完整能力审查，确认前置项完整且具备完全设计实现当前任务的能力。
>
> **触发时机**：在设计和开发过程中，发现设计的内容需要依赖底层实现或功能时。
>
> **执行者**：ARCH-A 主导审查，DEV-A 验证实现，QA-A 验证测试。

### 18.1 审查内容

当发现设计依赖底层实现时，必须审查以下内容：

| # | 审查项 | 要求 |
|---|--------|------|
| 1 | **基础设施能力** | 当前项目的基础设施能力是否完整且具备完全设计实现当前任务的能力？ |
| 2 | **前置项完整性** | 当前任务的前置项（依赖）是否完整？前置项的设计、开发、测试是否完善（编码、文档）？ |
| 3 | **其他依赖性问题** | 是否存在其他依赖性问题？ |
| 4 | **全面审查** | 如果存在缺失或问题，对整个基础设施与前置项进行全面审查（当发现一个 bug 时往往隐藏着更多问题） |

### 18.2 审查流程

```mermaid
flowchart TD
    A["发现设计依赖底层实现"] --> B["1. 审查基础设施能力"]
    B --> C{"能力完整？"}
    C -->|是| D["继续当前任务"]
    C -->|否| E["2. 审查前置项完整性"]
    E --> F{"前置项完整？"}
    F -->|是| D
    F -->|否| G["3. 全面审查基础设施与前置项"]
    G --> H["4. 修复缺失项"]
    H --> I["5. 再次审查"]
    I --> C
```
...
> **"直到审查不出问题为止"**：审查不是一次性活动，而是迭代过程。发现一个 bug 时往往隐藏着更多问题，必须持续审查直到确认无遗漏。

- **迭代审查**：每修复一个缺失项后，重新审查是否有新的依赖性问题
- **全面覆盖**：审查不仅看直接依赖，还要看间接依赖（当发现一个 bug 时往往隐藏着更多问题）
- **记录结果**：每次审查结果记录在开发文档中
...
> **目的**：每个 stage 阶段完成后必须压缩打包并返回包，确保用户可以获取每个阶段的完整交付物。
>
> **触发时机**：每个 stage 阶段完成且验收通过后（§3.2 验收全绿）。
>
> **执行者**：主导 Agent（通常为 ARCH-A 或 DEV-A）。

### 19.1 打包规则

| 规则 | 说明 |
|------|------|
| **时机** | 每个 stage 阶段完成且 §3.2 验收全绿后立即打包 |
| **格式** | tar.gz 压缩包 |
| **命名** | `landin-<stage0...>-v<big-version>.<min-version>.<fix-version>-<big-doc-stage-dev-plan-ver>.<small-doc-stage-dev-plan-ver>-<short-details>.tar.gz` |
| **内容** | 排除 `target/`、`.git/`、`download/` 目录 |
| **位置** | 保存到 `/home/z/my-project/download/` 目录 |
| **验证** | 打包后验证文件存在且大小合理（> 1MB） |

### 19.2 命名约定示例

```
landin-stage0-v0.492.0-stage18.275-process-doc-v7.0-optimize-r426.tar.gz
```

- `stage0` — 项目名称
- `v0.492.0` — 项目版本号
- `stage18.275` — 当前 stage 编号
- `process-doc-v7.0-optimize` — 简短描述
- `r426` — 修订号（递增）

### 19.3 打包命令

```bash
tar -czf /home/z/my-project/download/landin-stage0-v<version>-stage<stage>-<details>-r<N>.tar.gz \
  --exclude='target' --exclude='.git' --exclude='download' \
  -C /home/z/my-project landin-stage0
```
...
> **目的**：将"直到审查不出问题为止"的迭代审计原则固化为流程规则。审查不是一次性活动，而是持续迭代直到确认无遗漏。
>
> **触发时机**：
> 1. 发现一个 bug 或 soundness hole 时（§17.7 缺陷纳入）
> 2. 修复一个 bug 后（审查是否有同类 bug）
> 3. 修复一个 soundness hole 后（审查是否有类似 hole）
> 4. 任何深度审查（§14.5 / §14.6）完成后
>
> **执行者**：ARCH-A 主导，QA-A 验证。

### 20.1 核心原则

> **"直到审查不出问题为止"**：当发现一个 bug 时往往隐藏着更多问题。修复后必须继续审查同类路径，直到不再发现新问题。

**Bug 概率分布推理** (v7.4 新增)：
- 发现一个 bug 意味着**肯定存在大量类似 bug** — 因为 bug 的产生源于设计缺陷或实现模式错误，而设计/模式会在多处复用
- 遇到 bug 的阶段**肯定也大概率含有类似 bug** — 因为同一阶段的代码由同一模式生成，共享相同的缺陷模式
- Bug 的**显隐关系**：显性 bug（已触发）是冰山一角，隐性 bug（未触发但存在相同缺陷模式）往往更多
- 因此：**多方面全方位审查**是必要的 — 不仅仅是审查同类路径（§20.3），还要审查不同维度的同类模式（如所有传递 `None` 的位置、所有 `&cx` 签名的函数、所有 `loop {}` marker body）

### 20.2 迭代审计流程

```mermaid
flowchart TD
    A["发现 bug / soundness hole"] --> B["修复 bug"]
    B --> C["审查同类路径"]
    C --> D{"发现新问题？"}
    D -->|是| E["记录新问题"]
    E --> B
...
修复一个 bug 后，必须审查所有"同类路径"——即相同模式的其他代码路径：

| 发现的 bug 类型 | 需要审查的同类路径 |
|----------------|-------------------|
| expected-ty 未传播 | 所有接收 expected_ty 的代码路径（let binding, fn call arg, struct field, enum variant, fn return, if branch, match arm, array element, Box::new, struct literal field） |
| unify arg order 反转 | 所有 unify 调用点 |
| LOC 超阈值 | 所有源文件 LOC |
| 接口隔离违规 | 所有跨阶段调用 |
| 缺少 expected_ty 传播 | 所有表达式上下文 |
...
§17.7（缺陷纳入）定义了"发现 bug 时应纳入修复任务"的规则。§20（迭代审计）补充了"修复后继续审查同类路径直到不再发现新问题"的迭代原则。两者互补：
