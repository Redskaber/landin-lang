...
> **Author**: redskaber
> **Version**: 3.20 (effective from Stage 5.10)
> **Supersedes**: v3.19 (preserved verbatim — see §28.3 for v3.19→v3.20 diff)
...
| 准备开新阶段的 Agent | §2 MUV → §3 复杂度预评估 → §11 文档同步 → §18 轮次完成文档同步 → §9.3 阶段门审查 |
...
| 写测试 | §17.1, §17.5, §9.1 | tests/ 结构（§17.1）+ 矩阵覆盖（§17.5） |
| 写文档 | §11, §17.2, §17.7, §18.2 | 文档同步（§11）+ 格式（§17.7） |
| 审查代码 | §4, §15, §16, §21, §25 | 缺陷分级（§4）+ 深度审查（§25） |
| 重构 | §14, §15.3, §16 | 最优>最小（§15）+ 接口隔离（§16） |
...
process. All agents (main + subagents) must follow it. v3.20 effective
from Stage 5.10.**
