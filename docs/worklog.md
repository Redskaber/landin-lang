...
Task ID: stage18.132
Agent: Super Z (main) — ARCH-A + DEV-A + REV-A
Task: Stage 18.132 — TD-LOC-MIR-LOWER-EXPR 部分修复 (提取 call_lower.rs). v0.399.0 → v0.400.0.

Work Log:
- §3.1 环境检查: LLVM 19 + Rust 1.97.1 就绪 (Stage 18.127 已部署)
- §3.2 验收 (上个 stage 状态): cargo check ✅ + fmt ✅ + lib 640 ✅ + tests 2663 ✅
- §13.1 设计对齐: 查阅 docs/lang-design/ 06-mir.md (MIR body/place/ty 三层); call lowering 是 MIR lowering 中的明确概念

- §17 任务规划:
  → 选定 TD-LOC-MIR-LOWER-EXPR 继续修复 (expr_operand.rs 2503 LOC, 仍超 1500)
  → §13.4 J1-J6 判据检查:
    - J1 架构设计对齐 ✅ (灰区决策按子职责划分)
    - J2 单一职责 ✅ (call_lower.rs = call lowering 子职责)
    - J3 单向流动 ✅ (call_lower 被 expr_operand 调用; 递归调用 lower_expr_to_operand via super import)
    - J4 编译相关表达完整 ✅ (call lowering 子职责完整, 3 函数)
    - J5 阶段划分清晰 ✅ (全部在 mir::lower 阶段)
    - J6 科学合理粒度 ⚠️ (call_lower 362 ✅; expr_operand 2171 仍超 1500)
  → §12 最优 > 最小: 选择 call helpers 提取; MethodCall arm 提取因依赖复杂回退 (§2 原则 9 正确 > 妥协)

- 重构执行:
  → 拆分前: src/mir/lower/expr_operand.rs 2503 LOC (expression lowering + call helpers 混合)
  → 拆分后:
    - expr_operand.rs (2171 LOC): lower_expr_to_operand (2106 LOC) + imports
    - call_lower.rs (362 LOC): 3 call lowering helpers
  → 3 函数迁移到 call_lower.rs:
    - lower_expr_to_place (pub(super)) — 仅 expr_operand 调用
    - build_dyn_trait_call_terminator (pub) — mod.rs re-export (driver.rs 调用)
    - lower_closure_call_to_synthesized (pub(super)) — 仅 expr_operand 调用
  → mod.rs re-export 调整:
...
- MethodCall arm 提取尝试 + 回退:
  ...
  → 教训记录 (§14.5 D7): MethodCall arm 提取需更细致的类型签名验证, 推迟到 Stage 18.133
...
- §2.2 设计原则: 9/9 ✅
  → 原则 3 显式 > 隐式: 显式 re-export + pub(super) 标注
  → 原则 5 去除兼容思维: 不保留旧结构
  → 原则 6 通用 > 特例: 通用子职责划分 (call lowering)
  → 原则 9 正确 > 妥协: MethodCall arm 提取失败时回退而非强行 patch
...
- §3.2 验收 (全套通过):
  ...
  → cargo test --features llvm-backend --tests ✅ 2663 passed, 0 failed, 2 ignored (5.06s)

- §8 文档同步:
  → docs/develop/v0/stage-18/stage-18.132-dev-log.md (新建)
  → docs/develop/v0/tech-debt-register.md: v0.399.0 → v0.400.0 + TD-LOC-MIR-LOWER-EXPR 更新
  → docs/develop/v0/calibration-data.md: v0.7 → v0.8 + Stage 18.132 统计 + §2.3 流程优化历史
  → Cargo.toml: v0.399.0 → v0.400.0
  → README.md: v0.399.0 → v0.400.0
  → worklog.md (本条目)
...
Task ID: stage18.140
Agent: Super Z (main) — ARCH-A + DEV-A + REV-A
Task: Stage 18.140 — TD-LOC-DRIVER 继续修复 (提取 post-typeck validations). v0.407.0 → v0.408.0.

Work Log:
- §3.1 环境检查: LLVM 19 + Rust 1.97.1 就绪
- §3.2 验收 (上个 stage): cargo check ✅ + fmt ✅ + lib 640 ✅ + tests 2663 ✅
- §17 任务规划: 提取 post-typeck validation calls (80 LOC) 到 driver_codegen_prep.rs
- §13.4 J1-J6: J1-J5 全部通过; J6 部分通过 (mod.rs 1872 仍超 1500)
- 重构执行:
  → 提取 run_post_typeck_validations (validate_impls + validate_impl_method_signatures + validate_struct_literal_fields + validate_pattern_arity + validate_assignment_targets + validate_cast_types + builtin macro name registration)
  → 修复 clippy: 移除不必要的 & 引用 (函数参数已是引用)
  → 函数 pub(super) + 接收 hir/interner/errors/trait_resolver/fn_name_by_def_id
- §3.2 验收: 全套通过 (640 lib + 2663 integration, 0 failures)
- v0.408.0: patch bump
