...
| **16.00** | **v0.3 KICKOFF — Systematic full-stage audit complete. v0.2 COMPLETE: pipeline coverage 51/51, error system 50 sites, region inference all-pairs, sound Copy infrastructure ready. 3 remaining TODOs (all low priority). v0.3 roadmap: (1) sound Copy migration 2-3 days, (2) Task 3 TraitResolver keys 2 weeks, (3) Task 11 Monomorphization 2-3 weeks, (4) Task 10 Closure redesign 2-3 weeks.** | **0 (review only)** | ✅ |
| **16.01** | **TODO cleanup: lifetime tracking documentation — Resolved TODO in `lower_hir_ty_to_mir_ty_with_regions` by updating comment to reference `lower_hir_ty_to_mir_ty_with_lifetimes` (Stage 15.92). Remaining TODOs: 2 (both low priority).** | **0 (documentation only)** | ✅ |
| **16.02** | **Sound Copy migration attempt — Enabled `with_resolver_and_sigs` in driver: 247 failures (48 integration + 199 conformance). Reverted to `with_fn_sigs`. Sound path ready, test migration deferred to v0.3 (mechanical: add `impl Copy` to ~199 test structs). Complete migration plan documented. Per §1.0 原則 9.** | **0 (attempt + revert), 0 conformance changes** | ✅ |
| **16.03** | **Automated impl Copy migration script — Created `tools/migration/add_impl_copy.py` + `docs/tools/migration/add_impl_copy.md`. Ran on conformance tests: 393 `impl Copy` blocks added to 382 files. Tested with sound Copy: 117 failures remain (down from 247, 53% reduction). Remaining: 48 Rust test files + 69 complex .lin patterns need manual review. Reverted for v0.2 compat.** | **+393 impl Copy in .lin files (no test count change), 0 conformance changes** | ✅ |
| **16.04** | **Region error span tracking — Added `span` field to `RegionEscapesUniversal` error variant, populated from first matching constraint's cause span. Updated `borrowck/mod.rs` to use this span instead of `Span::DUMMY`. Resolves 2nd TODO (was 3 at Stage 16.00, now 1 remaining). Per §1.0 原則 4 "报错 > 静默" + §1.0 原則 3 "显式 > 隐式".** | **0 (no new tests), 0 conformance changes** | ✅ |
| **16.05** | **Field-not-found error reporting (last TODO resolution) — Changed `resolve_field_index` to take `cx: &mut MirLowerCtxt` so field-not-found errors are pushed directly to `cx.type_errors` (was silently dropped with `return 0`). Error message: `no field \`{name}\` on struct \`{struct}\``, span points to receiver. +6 integration tests, +1 conformance test flipped (limitation removed). 0 TODOs remaining in `src/`. Per §1.0 原則 4 "报错 > 静默" + §23 API 命名标准化.** | **+6 rust (2150 total), +1 conformance flipped (5224 total)** | ✅ |
| **16.06** | **Sound Copy detection ENABLED — Driver now uses `with_resolver_and_sigs` (was `with_fn_sigs`). Added field-level Copy derivation to TraitResolver: structs/enums with ALL Copy fields + no `impl Drop` are DERIVED Copy (mirrors Rust's `#[derive(Copy)]`). MIR lowerer uses `Operand::Move` for let bindings/returns/call args. `ty_is_copy` deprecated. +10 integration tests, 3 existing tests updated. Closes the sound Copy migration gap (was 117 failures). Per §1.0 原則 9 "正确 > 妥协" + §23 + §16.** | **+10 rust (2160 total), 0 conformance changes (5224 total)** | ✅ |
| **Total Stage 15+16** | **v0.2 COMPLETE + v0.3 in progress: 10/20 tasks COMPLETE, v0.2 RELEASE APPROVED. Systematic audit COMPLETE. Pipeline coverage 51/51. Error system 50 sites. Region inference all-pairs + span tracking. **Sound Copy: COMPLETE** (enabled in driver + field-level derivation + `ty_is_copy` deprecated). 0 TODOs remaining. 7628 tests, 0 failures, 0 warnings. v0.227.0!** | **+218 rust, +8 conformance** | ✅ |
