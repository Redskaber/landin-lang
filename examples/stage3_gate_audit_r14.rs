...
fn main() {
    let cases: &[Case] = &[
        ...
        Case { name: "r03_str_arg", src: "fn greet(s: &str) { } fn f() { greet(\"hello\") }", expect_all: &["define void @landin_greet(i8* %arg0)"], expect_none: &[], desc: "R13 r03: &str as function arg" },
        ...
        Case { name: "i09_struct_ref_str_field", src: "struct Wrap { s: &str } fn f(w: Wrap) { }", expect_all: &["{ i8* }"], expect_none: &[], desc: "Stage 3.47 §16.3.1: &str field in AdtLayout (no lower_hir_ty_to_mir_ty from codegen)" },
        ...
        Case { name: "e06_str_field_no_lower_call", src: "struct Wrap { s: &str } fn f(w: Wrap) -> &str { w.s }", expect_all: &["{ i8* }", "ret i8*"], expect_none: &[], desc: "Stage 3.47 edge: &str field read returns i8* (no codegen→MIR-lower call)" },
