...
fn test_dyn_trait_method_call_eq() {
    let c1 = DynTraitMethodCall::new("Clone", "S", "clone", 0, 0);
    let c2 = DynTraitMethodCall::new("Clone", "S", "clone", 0, 0);
    assert_eq!(c1, c2);

    let c3 = DynTraitMethodCall::new("Clone", "S", "clone_from", 1, 1);
    assert_ne!(c1, c3);
}
...
fn test_dyn_trait_method_call_clone() {
    let c1 = DynTraitMethodCall::new("Drop", "T", "drop", 0, 0);
    let c2 = c1.clone();
    assert_eq!(c1, c2);
}
...
fn test_dyn_trait_method_call_multiple_slots() {
    let fp = DynTraitFatPtr::new("Clone", "S");
    let call1 = DynTraitMethodCall::from_fat_ptr(&fp, "clone", 0, 0);
    let call2 = DynTraitMethodCall::from_fat_ptr(&fp, "clone_from", 1, 1);
