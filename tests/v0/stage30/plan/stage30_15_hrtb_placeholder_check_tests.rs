//! Stage 30.15 (v0.16 TD-HRTB-PLACEHOLDER-CHECK): HRTB placeholder universe
//! enforcement — reclassified as architectural work (v0.17+).
//!
...
//!
//! Per §1.0 原則 4 (报错 > 静默): HRTB bounds are partially enforced (Stage 30.13).
//! Per §1.0 原則 9 (正确 > 妥协): honest scope — full enforcement requires
//! InferCtxt with placeholder universes, which is an architectural change
//! deferred to TD-HRTB-INFRACTX-INTEGRATION (P2, v0.17+).
...
/// Stage 30.15 positive 1: HRTB bound with valid impl — compiles.
#[test]
fn stage30_15_positive_hrtb_valid_impl() {
    let result = compile(
        r#"
...
/// Stage 30.15 positive 2: HRTB in where clause with valid impl.
#[test]
fn stage30_15_positive_hrtb_where_clause() {
    let result = compile(
        r#"
trait Foo<'a> { fn foo(&self, x: &'a i32); }
struct MyType;
...
/// Stage 30.15 positive 3: Multiple HRTB bounds with valid impls.
#[test]
fn stage30_15_positive_multiple_hrtb() {
    let result = compile(
        r#"
trait Foo<'a> { fn foo(&self, x: &'a i32); }
...
/// Stage 30.15 positive 4: HRTB with multiple lifetime params.
#[test]
fn stage30_15_positive_hrtb_multiple_lifetimes() {
    let result = compile(
        r#"
...
/// Stage 30.15 regression 1: Non-HRTB bound still works.
#[test]
fn stage30_15_regression_non_hrtb_bound() {
    let result = compile(
        r#"
...
/// Stage 30.15 regression 2: HRTB with generic param (skipped — can't check).
#[test]
fn stage30_15_regression_hrtb_generic_param() {
    let result = compile(
        r#"
trait Foo<'a> { fn foo(&self, x: &'a i32); }
fn use_it<T: for<'a> Foo<'a>>(x: &T) {}
