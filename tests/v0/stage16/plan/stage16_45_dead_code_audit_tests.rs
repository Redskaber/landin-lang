//! Stage 16.45 — Project-wide dead code audit verification.

#![cfg(test)]
use landin_compiler::compile;

#[test]
fn stage16_45_basic_codegen() {
    let result = compile("fn main() -> i32 { 42 }");
    assert!(!result.has_errors());
}

#[test]
fn stage16_45_closure_codegen() {
    let result = compile("fn main() -> i32 { let f = |x| x + 1; f(10) }");
    assert!(!result.has_errors());
}

#[test]
fn stage16_45_triple_nested_closure() {
    let result = compile("fn main() -> i32 { let x = 1; let f = || || || x; let _ = f()()(); 42 }");
    assert!(!result.has_errors());
}

#[test]
fn stage16_45_trait_dispatch() {
...
#[test]
fn stage16_45_complete_program() {
    let src = r#"
