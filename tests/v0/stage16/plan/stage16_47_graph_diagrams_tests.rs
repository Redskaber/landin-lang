//! Stage 16.47 — Supplement docs/graph with error/type/trait system diagrams.

#![cfg(test)]
use landin_compiler::compile;

#[test]
fn stage16_47_basic_codegen() {
    let result = compile("fn main() -> i32 { 42 }");
    assert!(!result.has_errors());
}

#[test]
fn stage16_47_closure_codegen() {
    let result = compile("fn main() -> i32 { let f = |x| x + 1; f(10) }");
    assert!(!result.has_errors());
}

#[test]
fn stage16_47_trait_dispatch() {
...
#[test]
fn stage16_47_error_reporting() {
    let result = compile("fn main() -> i32 { let x = 1; x() }");
    assert!(!result.errors.typeck.is_empty());
}

#[test]
fn stage16_47_complete_program() {
    let src = r#"
