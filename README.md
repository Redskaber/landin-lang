...
|---|---|
| **Author** | redskaber |
| **Version** | v0.551.0 (v0.14 Stage 30.10 — TD-HRTB-SOLVER-INTEGRATION: HRTB bound collection + TD-HRTB-FULL-ENFORCEMENT created) |
| **License** | MIT |
| **Status** | ✅ **v0.14 COMPLETE**. 4921 tests (898 lib + 4023 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.14 tasks: ✅ Stage 30.6 TD-DROP-SCOPE-TIMING (scope tracking), ✅ Stage 30.7 TD-PROJECTION-IMPL-VERIFICATION (impl assoc type verification), ✅ Stage 30.8 TD-IMPL-TYPE-MATCH (structural check + TD-TYPECK-IMPL-CONTEXT created), ✅ Stage 30.9 TD-HRTB-FN-SYNTAX (Fn trait syntax), ✅ Stage 30.10 TD-HRTB-SOLVER-INTEGRATION (HRTB bound collection + TD-HRTB-FULL-ENFORCEMENT created). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-TYPECK-IMPL-CONTEXT (P2, v0.15+ typeck resolves `Self::Item`), TD-HRTB-FULL-ENFORCEMENT (P2, v0.15+ wire Binder into solver). Architecture health: 8.5/10 (183 files, 92,282 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax + HRTB bound collection). Next: v0.15 (typeck impl context + HRTB full enforcement). |
...
| TD-HRTB-FULL-ENFORCEMENT (NEW) | HRTB bounds collected but not enforced (no placeholder universes) | 🟡 P2, v0.15+ | Wire Binder<T> into trait solver selection + universes into region inference (Stage 30.10 created) |
