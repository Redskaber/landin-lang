...
|---|---|
| **Author** | redskaber |
| **Version** | v0.569.0 (v0.20 Stage 32.3 — TD-PRELUDE-MONO-ORDER RESOLVED via 4-point monomorphization fix — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.20 Stage 32.3 COMPLETE**. 5095 tests (898 lib + 4197 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §14.5 D1-D8 PASSED. Stage 32.3 implements the complete 4-point monomorphization fix: (1) `find_generics_for_fn_owner` returns impl+fn generics; (2) `resolve_self_param_type_for_sig` uses `lower_hir_ty_to_mir_ty_with_hir_and_generics`; (3) `resolve_self_param_type` in body_lower.rs + `cx.owner_def_id` field; (4) `resolve_trait_method` handles `Param(N)` via trait bounds. All callers updated (9 sites). compile_inner.rs Loops 1+2 + build_generics_map updated. 8 new tests (4 positive + 4 negative, 2 ignored for pre-existing typeck limits). Vec::push/get migration unblocked — Stage 32.4 follow-up. 3 new TD items documented (TD-SELF-OUTSIDE-IMPL-CONTEXT, TD-TYPECK-PARAM-RETURN-MISMATCH, TD-TYPECK-PARAM-ARG-COUNT — all P3). Architecture health: 9.85/10 (stable). Next: Stage 32.4 (Vec::push/get migration) + TD-FORMAT-ARGS. |
