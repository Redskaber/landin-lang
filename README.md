...
|---|---|
| **Author** | redskaber |
| **Version** | v0.588.0 (v0.27 Stage 39.3 — three root-cause fixes unblock prelude `Option::is_some`/`is_none`/`unwrap_or`; 5415 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.27 Stage 39.3 COMPLETE**. 5415 tests (898 lib + 4517 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 39.3 fixes three root-cause bugs that together blocked the prelude's `Option::is_some`/`is_none`/`unwrap_or` methods (and any prelude method using `match *self { Some(_) => ..., None => ... }` patterns). **TD-LEXER-UNDERSCORE**: `_` now produces `TokenKind::Underscore` (was `Ident("_")`), so `Some(_)` correctly parses as `Some(<wild>)`. **TD-PAT-IDENT-VARIANT**: resolver converts unit-variant-named Ident patterns (e.g., `None`) to Path patterns (was treated as catch-all binding). **TD-TEXT-IR-DEREF-ADT**: `detect_place_type` for Deref uses MIR type fallback when EmitType is `OpaquePtr` (fixes TextEmitter IR validity for `match *self { ... }`). Plus a binding sub-pattern classification fix (Ident is non-differentiating, same as Wild). Runtime verified: `Some(42).is_some() == true`, `None.is_some() == false`, `Some(42).unwrap_or(99) == 42`, `None.unwrap_or(99) == 99`. Architecture health: 9.85/10 (stable — three root-cause fixes, no regression). |
