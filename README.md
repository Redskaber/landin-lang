...
|---|---|
| **Author** | redskaber |
| **Version** | v0.548.0 (v0.14 Stage 30.7 — TD-PROJECTION-IMPL-VERIFICATION: validate impl blocks provide all required assoc types) |
| **License** | MIT |
| **Status** | ✅ **v0.14 Stage 30.7 — TD-PROJECTION-IMPL-VERIFICATION COMPLETE**. 4899 tests (898 lib + 4001 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.14 tasks: ✅ Stage 30.6 TD-DROP-SCOPE-TIMING (scope tracking), ✅ Stage 30.7 TD-PROJECTION-IMPL-VERIFICATION (validate impl blocks provide all required assoc types — missing assoc types now rejected with clear error). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-HRTB-SOLVER-INTEGRATION (P2, v0.14+ wire Binder into solver), TD-HRTB-FN-SYNTAX (P3, v0.14+ `for<'a> Fn(...)` syntax), TD-IMPL-TYPE-MATCH (P3, v0.14+ verify `type Item = T` matches method returns). Architecture health: 8.5/10 (183 files, 92,062 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification). Next: v0.14 continued (HRTB solver + HRTB Fn syntax + impl type match). |
...
| TD-IMPL-TYPE-MATCH (NEW) | `type Item = T` not verified against method returns `Self::Item` | 🟡 P3, v0.14+ | Verify type match after substitution (deferred from Stage 30.7) |
