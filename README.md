...
|---|---|
| **Author** | redskaber |
| **Version** | v0.569.0 (v0.20 Stage 32.3 — TD-PRELUDE-MONO-ORDER RESOLVED via 4-point monomorphization fix. Stage 32.4 Vec::push/get migration BLOCKED on v0.5+ method monomorphization — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.20 Stage 32.3 COMPLETE, Stage 32.4 BLOCKED**. 5095 tests (898 lib + 4197 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §14.5 D1-D8 PASSED. Stage 32.3 implements the complete 4-point monomorphization fix (find_generics_for_fn_owner + resolve_self_param_type_for_sig + resolve_self_param_type + resolve_trait_method on Param(N) via trait bounds). Stage 32.4 attempted Vec::push/get migration to prelude impl but ARCH-A vetoed: codegen doesn't substitute Param(N) in generic fn bodies, requiring v0.5+ method monomorphization (TD-VEC-PUSH-GET-MIGRATION, P2 BLOCKED). 4 new TD items documented (TD-VEC-PUSH-GET-MIGRATION + TD-SELF-OUTSIDE-IMPL-CONTEXT + TD-TYPECK-PARAM-RETURN-MISMATCH + TD-TYPECK-PARAM-ARG-COUNT — all v0.5+ architectural). Architecture health: 9.85/10 (stable). Next: v0.20 TD-FORMAT-ARGS (format! variadic args). |
