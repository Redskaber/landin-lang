...
|---|---|
| **Author** | redskaber |
| **Version** | v0.612.0 (v0.7 Stage 62 — TD-FN-TRAITS partial fix: Fn/FnMut/FnOnce traits + associated type Output added to prelude; manual impl pattern verified; closure auto-impl deferred to v0.8+; 5473 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.7 Stage 62 COMPLETE**. 5473 tests (898 lib + 4575 integration), 0 failures, 9 ignored. fmt clean, 0 clippy warnings. Stage 62 partially fixes TD-FN-TRAITS — `trait Fn<Args> { type Output; fn call(&self, args: Args) -> Self::Output; }` (and FnMut/FnOnce variants) added to prelude. Users can manually implement these traits for callable types and use `.call()`/`.call_mut()`/`.call_once()` syntax. Closure auto-impl (the common case `fn apply<F: Fn(i32) -> i32>(f: F)`) is deferred to v0.8+ (requires TyKind::Closure → Fn trait coercion in typeck + vtable emission). 6 new TDs discovered (all P3 v0.8+): TD-FN-CLOSURE-COERCION, TD-FN-UNIT-ARGS, TD-ASSOC-TYPE-SCOPE, TD-FN-IMPL-SIG-VALIDATION, TD-GENERIC-TRAIT-METHOD-MANGLING, TD-FN-ASSOC-TYPE-CALL. Runtime verified: `Doubler.call((21,))` → 42, `Counter.call_mut((5,))` → 15, `Consumer.call_once((41,))` → 42. Architecture health: 9.85/10 (stable — root-cause TD fix, no regression). |
