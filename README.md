...
|---|---|
| **Author** | redskaber |
| **Version** | v0.547.0 (v0.14 Stage 30.6 — TD-DROP-SCOPE-TIMING: scope tracking in MirLowerCtxt) |
| **License** | MIT |
| **Status** | ✅ **v0.14 Stage 30.6 — TD-DROP-SCOPE-TIMING COMPLETE**. 4889 tests (898 lib + 3991 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.14 task: ✅ Stage 30.6 TD-DROP-SCOPE-TIMING (scope tracking in MirLowerCtxt — StorageDead now emitted at block scope end, not function end; drop fires correctly for block-scoped locals). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-PROJECTION-IMPL-VERIFICATION (P2, v0.14+ impl block verification), TD-HRTB-SOLVER-INTEGRATION (P2, v0.14+ wire Binder into solver), TD-HRTB-FN-SYNTAX (P3, v0.14+ `for<'a> Fn(...)` syntax). Architecture health: 8.5/10 (183 files, 91,952 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing). Next: v0.14 continued (impl verification + HRTB solver + HRTB Fn syntax). |
