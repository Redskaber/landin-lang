...
|---|---|
| **Author** | redskaber |
| **Version** | v0.614.0 (v0.7 Stage 64 — TD-SPECIAL-16 FIXED: Drop trait added to prelude; drop glue infrastructure was already complete from Stage 15.x; 5496 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.7 Stage 64 COMPLETE**. 5496 tests (898 lib + 4598 integration), 0 failures, 14 ignored. fmt clean, 0 clippy warnings. Stage 64 fixes TD-SPECIAL-16 — `trait Drop { fn drop(&mut self); }` is now in the prelude. Users no longer need to declare `trait Drop` themselves — they can directly `impl Drop for MyType { fn drop(&mut self) { ... } }`. The drop glue infrastructure (drop_elaboration.rs + drop_glue.rs + is_drop_builtin + TerminatorKind::Drop) was already fully implemented in Stage 15.x — only the prelude declaration was missing. 13 test files updated (removed `trait Drop` declarations, TD-TRAIT-NAME-COLLISION workaround). 1 new TD discovered: TD-MEM-DROP (P3, v0.8+ — `mem::drop()` explicit drop function). Runtime verified: Drop called at scope exit, reverse order drops, nested scope drops. Architecture health: 9.85/10 (stable — root-cause TD fix, no regression). |
