...
|---|---|
| **Author** | redskaber |
| **Version** | v0.567.0 (v0.19 Stage 31.6g — Vec::push/get migration BLOCKED on prelude monomorphization; 4/7 intrinsics migrated — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.19 Stage 31.6g — Vec::push/get BLOCKED**. 5079 tests (898 lib + 4181 integration), 0 failures, 2 ignored. fmt clean, 0 clippy warnings. §14.5 D1-D8 PASSED. Stage 31.6g: Attempted Vec::push + Vec::get migration to prelude impl using sizeof(T) + realloc + pointer arithmetic + Deref store + bounds check panic. BLOCKED on prelude monomorphization order — `impl<T> Vec<T>` body lowered with T=Param(0) BEFORE monomorphization, causing typeck errors on arithmetic operations (`self.cap * 2`, `self.ptr + self.len`). Box::new works (operations consistent with Param), Vec::push/get fails (arithmetic needs concrete types). Per §18: root cause is prelude monomorphization (v0.5+ architectural change). Reverted, documented. Architecture health: 9.85/10 (stable). All P0/P1 soundness bugs resolved since v0.4. Next: Stage 31.7 — remove KNOWN_INTRINSIC_METHODS whitelist for migrated String/Box methods. |
