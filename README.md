...
|---|---|
| **Author** | redskaber |
| **Version** | v0.554.0 (v0.16 Stage 30.14 — TD-SELF-TYPE-RESOLUTION: Self::Item multi-segment path resolution + Projection lowering) |
| **License** | MIT |
| **Status** | ✅ **v0.16 Stage 30.14 — TD-SELF-TYPE-RESOLUTION PARTIAL COMPLETE**. 4939 tests (898 lib + 4041 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings (including tests). §14.5 D1-D8 PASSED. v0.16 task: ✅ Stage 30.14 TD-SELF-TYPE-RESOLUTION (Self::Item multi-segment path resolution in resolver + Projection lowering in ty_lower). v0.15 COMPLETE (Stage 30.12-30.13). v0.14 COMPLETE (Stage 30.6-30.10). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-HRTB-PLACEHOLDER-CHECK (P2, v0.16+ wire Binder + placeholder universes), TD-SELF-TYPE-SUBSTS (P3, v0.17+ fill substs[0] with Self type). Architecture health: 8.5/10 (183 files, 92,492 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax + HRTB bound collection + assoc type bindings + pre-typeck projection + HRTB partial enforcement + Self::Item multi-segment path resolution). Next: v0.16 continued (HRTB placeholder check) or v0.17 (Self type substs). |
...
| TD-HRTB-PLACEHOLDER-CHECK (NEW) | HRTB bounds partially enforced — no universal quantification check | 🟡 P2, v0.16+ | Wire Binder<T> + enter_universe/exit_universe for placeholder regions (Stage 30.13 created) |
