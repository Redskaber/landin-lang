...
|---|---|
| **Author** | redskaber |
| **Version** | v0.579.0 (v0.24 Stage 36.5 COMPLETE — TD-ARRAY-SLICE-RUNTIME-COERCION-MISSING RESOLVED; runtime array→slice fat pointer construction in codegen; TD-FORMAT-MIGRATION now UNBLOCKED — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.24 Stage 36.5 COMPLETE**. 5293 tests (898 lib + 4395 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 36.5 resolves TD-ARRAY-SLICE-RUNTIME-COERCION-MISSING — two coordinated codegen changes: (1) Rvalue::Ref constructs fat pointer {ptr, len=N} for Array-typed places, (2) mir_type_to_emit_type maps Ref(Array) to fat pointer struct {ptr, i64} (same as Ref(Slice)), ensuring correct alloca sizing. Runtime verified: s.len() returns correct array length (3 for [i64; 3]). TD-FORMAT-MIGRATION (P2) is now UNBLOCKED — all prerequisites resolved (Stage 36.1 slice len + coercion, Stage 36.4 elem type resolution, Stage 36.5 runtime fat pointer). Architecture health: 9.85/10 (stable). Next: Stage 36.6 (retry TD-FORMAT-MIGRATION — slice-based prelude format impl). |
