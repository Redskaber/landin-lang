...
|---|---|
| **Author** | redskaber |
| **Version** | v0.589.0 (v0.28 Stage 40.1 — prelude `Option::map`/`and_then` + `Result::map`/`and_then` combinators added; 5436 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.28 Stage 40.1 COMPLETE**. 5436 tests (898 lib + 4538 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 40.1 adds the four most-requested combinator methods to the Landin prelude: `Option::map<U>(self, f: fn(T) -> U) -> Option<U>`, `Option::and_then<U>(self, f: fn(T) -> Option<U>) -> Option<U>`, `Result::map<U>(self, f: fn(T) -> U) -> Result<U, E>`, `Result::and_then<U>(self, f: fn(T) -> Result<U, E>) -> Result<U, E>`. These were unblocked by Stage 39.3's three root-cause fixes (TD-LEXER-UNDERSCORE, TD-PAT-IDENT-VARIANT, TD-TEXT-IR-DEREF-ADT). Per §1.0 原則 6 (通解 > 特解): one generic mechanism handles all transform functions via `fn(T) -> U` parameters, no special-case intrinsics. Per §12 (最优 > 最小): root-cause fix at prelude level using standard match dispatch. Runtime verified: `Some(21).map(double) == Some(42)`, `None.map(double) == None`, `Some(42).and_then(half_even) == Some(21)`, `Ok(21).map(double) == Ok(42)`, `Err(99).map(double) == Err(99)` (propagates), `Ok(42).and_then(half_even) == Ok(21)`, `Err(99).and_then(half_even) == Err(99)` (propagates). Architecture health: 9.85/10 (stable — prelude extension, no regression). |
