...
|---|---|
| **Author** | redskaber |
| **Version** | v0.578.0 (v0.24 Stage 36.4 COMPLETE — TD-ARRAY-ELEMENT-TYPE-RESOLUTION RESOLVED; writeback pipeline now resolves array element types from Infer to concrete; 5 positive + 28 negative tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.24 Stage 36.4 COMPLETE**. 5260 tests (898 lib + 4362 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 36.4 resolves TD-ARRAY-ELEMENT-TYPE-RESOLUTION — array literal `[1, 2, 3]` had element type `Infer` at codegen time because `compute_writeback_ty` only handled `AggregateKind::Tuple`, not `AggregateKind::Array`. Fix: added Array Aggregate writeback rule that takes the first operand's resolved type as the element type. This is the root-cause fix for the "unresolved type kind" warnings seen in Stage 36.3. TD-FORMAT-MIGRATION (P2) still BLOCKED on TD-ARRAY-SLICE-RUNTIME-COERCION-MISSING (codegen local allocation infrastructure — locals allocated as `ptr` not `{ptr, i64}`). Architecture health: 9.85/10 (stable). Next: fix codegen local allocation for fat pointer types. |
