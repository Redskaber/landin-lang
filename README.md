...
|---|---|
| **Author** | redskaber |
| **Version** | v0.591.0 (v0.28 Stage 40.3 — TD-UNREACHABLE-MACRO-BROKEN fix + Option::or/or_else/filter + 4-dimension architecture audit; 5436 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.28 Stage 40.3 COMPLETE**. 5436 tests (898 lib + 4538 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 40.3 performs a 4-dimension architecture audit (type system, special-case→general, runtime.rs C wrappers, macro system) and fixes a P1 bug (TD-UNREACHABLE-MACRO-BROKEN — `unreachable!` macro had same `.ptr` extraction bug as `panic!`). Adds `Option::or` / `Option::or_else` / `Option::filter` combinators. Runtime verified: `unreachable!("msg")` → `internal error: entered unreachable code: msg`, `None.or(Some(99))` → `99`, `None.or_else(make_default)` → `42`, `Some(4).filter(is_even)` → `4`. Audit identified 15 P2/P3 TDs for v0.5+/v0.6+ roadmap. Architecture health: 9.85/10 (stable — audit-driven root-cause fix + prelude extension, no regression). |
