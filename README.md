...
|---|---|
| **Author** | redskaber |
| **Version** | v0.571.0 (v0.21 Stage 33.1 — Recursive collect_param_bindings + type_name_by_def_id threading + substitute Load/GEP/Store for method monomorphization infrastructure. Vec::push/get migration BLOCKED on TD-IMPL-METHOD-GENERIC-PARAM-RESOLUTION — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.21 Stage 33.1 COMPLETE** (infrastructure improvements). 5095 tests (898 lib + 4197 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §14.5 D1-D8 PASSED. Stage 33.1 implements method monomorphization infrastructure: (1) recursive `collect_param_bindings` handles nested generic types (Vec<T>, Option<T>); (2) `type_name_by_def_id` threaded through all codegen paths (fixes call-site ↔ definition name mismatch); (3) `substitute_rvalue_types` now handles Load + GEP + Store (fixes Param leak in generic fn bodies). Vec::push/get migration attempted but BLOCKED on TD-IMPL-METHOD-GENERIC-PARAM-RESOLUTION (P2, v0.5+) — resolver doesn't resolve `value: T` in impl method signatures. Architecture health: 9.85/10 (stable). Next: v0.5+ resolver fix to unblock Vec::push/get + format! migrations. |
