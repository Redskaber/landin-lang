...
|---|---|
| **Author** | redskaber |
| **Version** | v0.626.0 (v0.8 Stage 86 — TD-FN-IMPL-SIG-VALIDATION return type check FIXED: typeck now validates impl method return type against trait's `Self::Output` assoc type projection; 3 sites fixed (driver_validations_impl + projection_resolver pub + ty_lower find_assoc_type_def_id_in_trait); 5540 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.8 Stage 86 COMPLETE**. 5540 tests (898 lib + 4642 integration), 0 failures, 9 ignored. fmt clean, 0 clippy warnings. Stage 86 fixes TD-FN-IMPL-SIG-VALIDATION (return type check): typeck now validates the impl method's return type against the trait's declared return type — including when the trait return type is `Self::Output` (an associated type projection). Three fixes: (1) `driver_validations_impl.rs` uses HIR-aware ty lowering + `resolve_projection_in_ty_pub` for both impl_ret and trait_ret (was: `lower_hir_ty_to_mir_ty` without HIR context → `Self::Output` became `Error` → `mir_ty_kinds_compatible(_, Error) == true` → silently accepted mismatches); (2) `projection_resolver.rs` exposed `resolve_projection_in_ty_pub` as pub alias; (3) `ty_lower.rs` fixed `find_assoc_type_def_id` to match by name AND owner trait (was: name only → `Self::Output` in `FnMut::call_mut` found `Fn`'s Output, not FnMut's → resolver looked for impls of wrong trait → Projection stayed unresolved). 4 new tests (1 positive + 3 negative typeck return type mismatch) + 1 test un-ignored (stage62_fn_trait_wrong_return_type_errors). Architecture health: 9.85/10 (stable — closed assoc type projection gap). |
