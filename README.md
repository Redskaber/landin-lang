...
|---|---|
| **Author** | redskaber |
| **Version** | v0.625.0 (v0.8 Stage 85 — TD-FN-UNIT-ARGS FIXED: `Fn<()>` unit tuple arg now correctly elided from LLVM forward declarations; `build_fn_sigs_map` filters `EmitType::Void` from sig map, mirroring ZST elision in codegen_function + terminator.rs; 5535 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.8 Stage 85 COMPLETE**. 5535 tests (898 lib + 4637 integration), 0 failures, 10 ignored. fmt clean, 0 clippy warnings. Stage 85 fixes TD-FN-UNIT-ARGS: `Fn<()>` (empty tuple as Args) now correctly supported. The root cause was in `build_fn_sigs_map` (src/codegen/llvm/function_sigs.rs) — it built forward-declaration signatures from `sig.inputs` WITHOUT filtering `EmitType::Void` (which is what `()` maps to). So `impl Fn<()> for Getter { fn call(&self, args: ()) -> i32 { ... } }` failed at LLVM module verification with "Function arguments must have first-class types! void %0" (forward decl had `void` param while actual definition elided the ZST). Fixed by adding `.filter(|ty| *ty != EmitType::Void)` to the param_tys builder — mirroring the ZST elision already done in `codegen_function` (Stage 18.335) and `terminator.rs` call site (Stage 18.335). This is the **third site** needing the same ZST elision fix (definition + call site + forward decl). 4 new tests (1 positive runtime + 3 negative typeck) + 1 test un-ignored (stage62_fn_trait_unit_arg). Architecture health: 9.85/10 (stable — closed ZST elision gap). |
