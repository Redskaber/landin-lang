...
|---|---|
| **Author** | redskaber |
| **Version** | v0.549.0 (v0.14 Stage 30.8 — TD-IMPL-TYPE-MATCH: structural type match check + TD-TYPECK-IMPL-CONTEXT created) |
| **License** | MIT |
| **Status** | ✅ **v0.14 Stage 30.8 — TD-IMPL-TYPE-MATCH PARTIAL COMPLETE**. 4905 tests (898 lib + 4007 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.14 tasks: ✅ Stage 30.6 TD-DROP-SCOPE-TIMING (scope tracking), ✅ Stage 30.7 TD-PROJECTION-IMPL-VERIFICATION (impl assoc type verification), ✅ Stage 30.8 TD-IMPL-TYPE-MATCH (structural check — no-op for common case; deeper typeck issue tracked as TD-TYPECK-IMPL-CONTEXT P2 v0.15+). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-HRTB-SOLVER-INTEGRATION (P2, v0.14+ wire Binder into solver), TD-HRTB-FN-SYNTAX (P3, v0.14+ `for<'a> Fn(...)` syntax), TD-TYPECK-IMPL-CONTEXT (P2, v0.15+ typeck resolves `Self::Item` during method body checking). Architecture health: 8.5/10 (183 files, 92,122 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + structural type match). Next: v0.14 continued (HRTB solver + HRTB Fn syntax) or v0.15 (typeck impl context). |
...
| TD-TYPECK-IMPL-CONTEXT (NEW) | typeck doesn't resolve `Self::Item` to `T` during method body checking | 🟡 P2, v0.15+ | Add impl-block context to typeck (Stage 30.8 created) |
...
| TD-HRTB-SOLVER-INTEGRATION | HRTB `for<'a>` surface syntax captured but solver doesn't enforce semantics | 🟡 v0.14+ | Wire Binder<T> into trait solver selection + universe into region inference (Stage 30.5 created) |
| TD-HRTB-FN-SYNTAX | `for<'a> Fn(&'a T) -> &'a U` syntax not parsed (Fn call syntax separate feature) | 🟡 v0.14+ | Implement Fn(...) trait call syntax (Stage 30.5 created) |
