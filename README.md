...
|---|---|
| **Author** | redskaber |
| **Version** | v0.568.0 (v0.19 COMPLETE → v0.20 START — 4/7 intrinsics migrated, 3 BLOCKED on v0.5+/v0.20+ — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.19 COMPLETE → v0.20 START**. 5087 tests (898 lib + 4189 integration), 0 failures, 2 ignored. fmt clean, 0 clippy warnings. §14.5 D1-D8 PASSED. v0.19 Stage 31 series (10 stages): 4/7 TD-INTRINSIC-OVERUSE intrinsics migrated from MIR intrinsic (特解) to prelude impl (通解) — String::as_str (FatPtrLit), String::from_str (.ptr/.len + extern C), String::push_str (while + realloc), Box::new (sizeof + alloc + Deref). 3 new language features: FatPtrLit `&str { ptr, len }`, fat pointer field access `.ptr`/`.len`, `sizeof TYPE`. 3/7 BLOCKED on v0.5+/v0.20+: Vec::push/get (prelude monomorphization), format! (format args). Net -138 LOC. All resolvable tech-debt at current architecture RESOLVED. Remaining TD ALL BLOCKED on architectural changes (properly documented per §1.0 原則 9). Architecture health: 9.85/10 (stable throughout v0.19). Next: v0.20 — prelude monomorphization (Priority 1) + format args (Priority 2). |
