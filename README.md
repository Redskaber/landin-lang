...
|---|---|
| **Author** | redskaber |
| **Version** | v0.629.0 (v0.8 Stage 89 — TD-DYN-TRAIT-FAT-PTR-COERCION call site fat pointer FIXED: call site now passes `@.dynptr.Trait.Concrete` fat pointer global when callee expects `&dyn Trait`; fix in codegen/terminator.rs + build_type_name_by_def_id; 5548 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.8 Stage 89 COMPLETE**. 5548 tests (898 lib + 4650 integration), 0 failures, 9 ignored. fmt clean, 0 clippy warnings. Stage 89 fixes TD-DYN-TRAIT-FAT-PTR-COERCION (call site fat pointer): `&ConcreteType → &dyn Trait` coercion at call sites now passes the fat pointer global `@.dynptr.Trait.Concrete` instead of the thin data pointer. Fix in `codegen/terminator.rs`: when building call args, detect if callee param is `Ref(_, _, Dyn(trait_def_id))` and arg is `Ref(_, _, Adt(concrete_def_id))` — if so, construct the dynptr symbol `@.dynptr.{trait_name}.{concrete_name}` and pass it. Also fixed `build_type_name_by_def_id` to include Trait DefIds (was: only Struct/Enum). IR verification: `main` now passes `call i32 @landin_use_greeter(ptr @.dynptr.Greeter.English)` (was: `ptr %v1` thin pointer). 4 new tests (1 positive + 3 negative). New TD: TD-DYN-TRAIT-DATA-PTR-EXTRACT (v0.9+) — vtable indirect call passes fat pointer to impl method (expects thin ptr to data). Architecture health: 9.85/10 (stable — call site fat pointer wired). |
