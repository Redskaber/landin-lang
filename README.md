...
```
landin-stage0/
├── Cargo.toml              v0.11.19 (autotests=false — single all_tests target)
├── src/
...
│   └── bin/                CLI entry point
├── tests/
│   ├── all_tests.rs        Unified entry point (38 #[path] mod declarations)
...
- **Stage 5** 🔄 In progress (5.1-5.20 done; next: dyn Trait MIR lowering, full stdlib, mini-cargo)
