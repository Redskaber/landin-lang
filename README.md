...
|---|---|
| **Author** | redskaber |
| **Version** | v0.590.0 (v0.28 Stage 40.2 — TD-PANIC-MACRO-BROKEN fix + Option/Result unwrap/expect added; 5436 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.28 Stage 40.2 COMPLETE**. 5436 tests (898 lib + 4538 integration), 0 failures, 4 ignored. fmt clean, 0 clippy warnings. §3.2 verification passed. Stage 40.2 fixes a P1 bug (TD-PANIC-MACRO-BROKEN) where the `panic!` macro was registered but never usable (missing `__landin_panic_msg` extern declaration), plus TD-PANIC-MACRO-STR-PTR (macro body passed `&str` instead of `*const u8`) and TD-PANIC-MACRO-HYGIENE-FIELD (hygiene renamed struct field names `ptr`/`len`/`cap`). Adds `Option::unwrap()` / `Option::expect(msg)` / `Result::unwrap()` / `Result::expect(msg)` to prelude. Runtime verified: `Some(42).unwrap() == 42`, `None.unwrap()` panics with descriptive message, `Ok(42).unwrap() == 42`, `Err(99).unwrap()` panics, `panic!("msg")` prints `panic: msg` and exits. Architecture health: 9.85/10 (stable — 3 root-cause fixes + prelude extension, no regression). |
