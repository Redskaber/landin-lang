...
|---|---|
| **Author** | redskaber |
| **Version** | v0.550.0 (v0.14 Stage 30.9 — TD-HRTB-FN-SYNTAX: `Fn(T) -> U` trait bound syntax) |
| **License** | MIT |
| **Status** | ✅ **v0.14 Stage 30.9 — TD-HRTB-FN-SYNTAX COMPLETE**. 4915 tests (898 lib + 4017 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.14 tasks: ✅ Stage 30.6 TD-DROP-SCOPE-TIMING (scope tracking), ✅ Stage 30.7 TD-PROJECTION-IMPL-VERIFICATION (impl assoc type verification), ✅ Stage 30.8 TD-IMPL-TYPE-MATCH (structural check + TD-TYPECK-IMPL-CONTEXT created), ✅ Stage 30.9 TD-HRTB-FN-SYNTAX (`Fn(T) -> U`/`FnMut(T) -> U`/`FnOnce(T) -> U` trait bound syntax — including HRTB `for<'a> Fn(&'a T) -> &'a U`). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-HRTB-SOLVER-INTEGRATION (P2, v0.14+ wire Binder into solver), TD-TYPECK-IMPL-CONTEXT (P2, v0.15+ typeck resolves `Self::Item`). Architecture health: 8.5/10 (183 files, 92,192 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax). Next: v0.14 final (TD-HRTB-SOLVER-INTEGRATION) or v0.15 (typeck impl context). |
