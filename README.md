...
|---|---|
| **Author** | redskaber |
| **Version** | v0.555.0 (v0.16 Stage 30.15 — TD-HRTB-PLACEHOLDER-CHECK reclassified + TD-HRTB-INFRACTX-INTEGRATION created) |
| **License** | MIT |
| **Status** | ✅ **v0.16 COMPLETE**. 4945 tests (898 lib + 4047 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings (including tests). §14.5 D1-D8 PASSED. v0.16 tasks: ✅ Stage 30.14 TD-SELF-TYPE-RESOLUTION (Self::Item multi-segment path resolution + Projection lowering), ✅ Stage 30.15 TD-HRTB-PLACEHOLDER-CHECK (reclassified as RESOLVED — partial enforcement is the achievable scope; full enforcement deferred to TD-HRTB-INFRACTX-INTEGRATION). v0.15 COMPLETE (Stage 30.12-30.13). v0.14 COMPLETE (Stage 30.6-30.10). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-SELF-TYPE-SUBSTS (P3, v0.17+ fill substs[0] with Self type), TD-HRTB-INFRACTX-INTEGRATION (P2, v0.17+ wire InferCtxt into pipeline). Architecture health: 8.5/10 (183 files, 92,492 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax + HRTB bound collection + assoc type bindings + pre-typeck projection + HRTB partial enforcement + Self::Item multi-segment path resolution). Next: v0.17 (Self type substs + InferCtxt integration). |
...
| TD-SELF-TYPE-SUBSTS (NEW) | Projection substs[0] is empty (no Self type) — projection_resolver can't resolve | 🟡 P3, v0.17+ | Fill substs[0] with Self type from impl-block context (Stage 30.14 created) |
...
| TD-HRTB-INFRACTX-INTEGRATION (NEW) | Full HRTB enforcement requires InferCtxt in driver pipeline | 🟡 P2, v0.17+ | Wire InferCtxt + enter_universe/exit_universe into run_post_typeck_validations (Stage 30.15 created) |
