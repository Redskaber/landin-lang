...
|---|---|
| **Author** | redskaber |
| **Version** | v0.630.0 (v0.8 Stage 90 — TD-DYN-TRAIT-DATA-PTR-EXTRACT FIXED: dyn Trait method calls now extract data pointer from fat pointer and pass to impl method; **first successful end-to-end dyn Trait runtime** — `use_greeter(&e)` returns 42; 5556 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.8 Stage 90 COMPLETE**. 5556 tests (898 lib + 4658 integration), 0 failures, 9 ignored. fmt clean, 0 clippy warnings. Stage 90 fixes TD-DYN-TRAIT-DATA-PTR-EXTRACT (vtable indirect call data pointer): the vtable indirect call now extracts the data pointer from the fat pointer field 0 and passes it to the impl method (was: passed fat pointer → method read garbage → returned 0). Fix in `codegen/llvm/aggregate.rs` + `codegen/text/aggregate.rs`: GEP field 0 + load data pointer before the indirect call. **This completes the dyn Trait end-to-end chain**: typeck (Stage 87) → vtable dispatch (Stage 88) → call site fat pointer (Stage 89) → data pointer extract (Stage 90). First successful runtime test: `use_greeter(&e)` where `English::greet` returns 42 now correctly exits with 42. 4 new tests (1 positive runtime + 3 negative). Architecture health: 9.85/10 (stable — dyn Trait end-to-end works). |
