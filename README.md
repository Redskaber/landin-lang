...
|---|---|
| **Author** | redskaber |
| **Version** | v0.546.0 (v0.13 Stage 30.5 — TD-GAT-HIGHER-RANKED partial implementation: HRTB `for<'a>` parser + AST + HIR) |
| **License** | MIT |
| **Status** | ✅ **v0.13 COMPLETE**. 4881 tests (898 lib + 3983 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings on lib. §14.5 D1-D8 PASSED. v0.13 tasks: ✅ Stage 30.2 TD-STUB-LIFETIME-ELISION-NOOP (RFC 141 Rule 4 enforcement), ✅ Stage 30.3 TD-STUB-DROP-ELABORATION-NOOP (reclassified as RESOLVED), ✅ Stage 30.4 TD-STUB-PROJECTION-RESOLVER (reclassified as RESOLVED), ✅ Stage 30.5 TD-GAT-HIGHER-RANKED (partial implementation — HRTB `for<'a>` surface syntax layer: parser + AST + HIR). Remaining TDs ALL BLOCKED: TD-DROP-SCOPE-TIMING (P2, v0.14+ scope tracking), TD-PROJECTION-IMPL-VERIFICATION (P2, v0.14+ impl block verification), TD-HRTB-SOLVER-INTEGRATION (P2, v0.14+ wire Binder into solver + universe into region inference), TD-HRTB-FN-SYNTAX (P3, v0.14+ `for<'a> Fn(...)` syntax). Architecture health: 8.5/10 (183 files, 91,892 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax). Next: v0.14 (scope tracking + impl verification + HRTB solver integration). |
