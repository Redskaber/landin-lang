...
|---|---|
| **Author** | redskaber |
| **Version** | v0.556.0 (v0.17 Stage 30.16 — TD-SELF-TYPE-SUBSTS: empty-substs fallback in projection_resolver) |
| **License** | MIT |
| **Status** | ✅ **v0.17 Stage 30.16 — TD-SELF-TYPE-SUBSTS COMPLETE**. 4951 tests (898 lib + 4054 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings (including tests). §14.5 D1-D8 PASSED. v0.17 task: ✅ Stage 30.16 TD-SELF-TYPE-SUBSTS (empty-substs fallback in projection_resolver — Self::Item now resolves to concrete type from impl block). v0.16 COMPLETE (Stage 30.14-30.15). v0.15 COMPLETE (Stage 30.12-30.13). v0.14 COMPLETE (Stage 30.6-30.10). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-HRTB-INFRACTX-INTEGRATION (P2, v0.17+ wire InferCtxt into pipeline). Architecture health: 8.5/10 (183 files, 92,552 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax + HRTB bound collection + assoc type bindings + pre-typeck projection + HRTB partial enforcement + Self::Item multi-segment path resolution + Self::Item empty-substs fallback). Next: v0.17 continued (InferCtxt integration for full HRTB enforcement). |
