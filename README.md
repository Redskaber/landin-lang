...
|---|---|
| **Author** | redskaber |
| **Version** | v0.613.0 (v0.7 Stage 63 — TD-IMPL-TRAIT partial fix: impl Trait in arg position desugared to generic param at HIR lowering; method calls inside body deferred to v0.8+; 5482 tests — Architecture health 9.85/10) |
| **License** | MIT |
| **Status** | ✅ **v0.7 Stage 63 COMPLETE**. 5482 tests (898 lib + 4584 integration), 0 failures, 13 ignored. fmt clean, 0 clippy warnings. Stage 63 partially fixes TD-IMPL-TRAIT — `fn f(x: impl Trait)` is now desugared to `fn f<__impl_T_N: Trait>(x: __impl_T_N)` at HIR lowering time (the canonical Rust approach per Rust Reference §6.3). The rest of the pipeline (typeck, MIR lowering, codegen) handles it as a regular generic param. Method calls inside the function body (e.g., `x.clone()`) are deferred to v0.8+ (TD-IMPL-TRAIT-MONO-RESOLUTION — monomorphization doesn't re-resolve trait methods after type substitution). 4 new TDs discovered (1 P1 + 3 P3, v0.8+): TD-IMPL-TRAIT-MONO-RESOLUTION, TD-IMPL-TRAIT-CALLSITE-CHECK, TD-IMPL-TRAIT-UNDEFINED-BOUND, TD-IMPL-TRAIT-NO-BOUNDS. Runtime verified: `process(7)` with `impl Clone` arg compiles and runs. Architecture health: 9.85/10 (stable — root-cause TD fix, no regression). |
