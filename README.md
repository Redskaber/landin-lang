...
|---|---|
| **Author** | redskaber |
| **Version** | v0.553.0 (v0.15 Stage 30.13 — TD-HRTB-FULL-ENFORCEMENT: HRTB partial enforcement + TD-HRTB-PLACEHOLDER-CHECK created) |
| **License** | MIT |
| **Status** | ✅ **v0.15 COMPLETE**. 4933 tests (898 lib + 4035 integration), 0 failures, 2 ignored (`ulimit -s unlimited`, single-thread). fmt clean, 0 clippy warnings (including tests). §14.5 D1-D8 PASSED. v0.15 tasks: ✅ Stage 30.12 TD-TYPECK-IMPL-CONTEXT (assoc type bindings + pre-typeck projection + warning fixes), ✅ Stage 30.13 TD-HRTB-FULL-ENFORCEMENT (HRTB partial enforcement + TD-HRTB-PLACEHOLDER-CHECK created). v0.14 COMPLETE (Stage 30.6-30.10). v0.13 COMPLETE (Stage 30.2-30.5). Remaining TDs ALL BLOCKED: TD-SELF-TYPE-RESOLUTION (P2, v0.16+ deeper HIR self type resolution), TD-HRTB-PLACEHOLDER-CHECK (P2, v0.16+ wire Binder + placeholder universes). Architecture health: 8.5/10 (183 files, 92,412 LOC). All P0/P1 soundness bugs resolved since v0.4. All language feature enforcement complete (visibility + break/continue + enum exhaustiveness + lifetime elision rule 4 + HRTB surface syntax + drop scope timing + impl assoc type verification + Fn trait syntax + HRTB bound collection + assoc type bindings + pre-typeck projection + HRTB partial enforcement). Next: v0.16 (self type resolution + HRTB placeholder check). |
...
| TD-SELF-TYPE-RESOLUTION (NEW) | Self::Item resolution may not fully work (deeper HIR self type needed) | 🟡 P2, v0.16+ | Deeper HIR self type resolution for full Self::Item (Stage 30.12 created) |
